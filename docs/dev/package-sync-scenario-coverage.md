...
- A snap Nomad holds no revision of at all (E115): the one VM run under this article asserts the boundary inside an app Nomad does hold (E113), so no two-machine run shows a declined or failed install leaving the app's whole data directory home. It is the same read of Nomad's `snap list` and the same exclusion either way, so what is missing is a second shape of one mechanism, not a second mechanism.
