...
| F100 | Atlas's own filter denies an application Atlas has installed from that remote | The run ends naming the application, the remote and the filter, before anything is asked or written: a machine that says both "install this from there" and "nothing from there may be installed" is contradicting itself, and no filter comes off to make the install possible | U | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_filter_that_denies_an_app_the_source_itself_has_ends_the_run` |
...
| F151 | Atlas's filter file cannot be read, or is one flatpak itself would reject | The run does not end: nothing can be said about what such a file denies, so the applications proceed and flatpak's own error reaches the user when the filter is applied | U | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_filter_file_flatpak_would_reject_does_not_end_the_run` |
...
The register and the marks are one statement, read together. A row marked `P` is either an entry here saying what is still missing, or a composition listed in `COMPOSITIONS` in `tests/unit/test_scenario_coverage_matrix.py`, whose Test column names the rows it is read through — D61, F121, H133 and J107 are the compositions; K67 and K89 are the two `P` rows with an entry. A row marked `U` carrying an entry claims exactly what it says: the unit tier proves the branch, and the VM run its own wording asks for does not exist.
...
- **K89** — that `home.filter`'s `- .cache/*` excludes `.cache/pc-switcher`, the directory apt's staging copy lives in. The transfer half is audited statically and the staging copy's removal is asserted on both paths; what is left is the filter rule nobody reads against the path it has to cover. Unit.
