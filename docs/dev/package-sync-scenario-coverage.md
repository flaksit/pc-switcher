...
| C10 | `/etc/apt/sources.list` on Nomad and not on Atlas | never offered for deletion | P | `test_apt_probe:TestWhatAptItselfReads::test_sources_list_is_digested_on_both_machines_and_is_still_not_an_item` |
...
| C176 | `vendor.list`'s last user's removal is approved, so the deletion is offered — and a collateral question answered on the same screens then withdraws that removal | the file is not deleted: the count that raised it no longer holds, and an approval taken back is logged rather than applied | — | — (see Gaps) |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| E36 | An approved removal of a snap from Nomad | The snap is removed and snapd's own pre-removal snapshot is left in place | U V | `test_snap_sync:TestConvergeRemoval::test_removal_never_passes_purge`; `test_package_sync:TestSnapRemovalKeepsSnapdsSnapshot::test_a_removal_through_a_sync_leaves_a_snapshot_snap_saved_lists` |
| E37 | After a real removal through a sync, the user runs `snap saved` on Nomad | The snapshot for the removed snap is listed | V | `test_package_sync:TestSnapRemovalKeepsSnapdsSnapshot::test_a_removal_through_a_sync_leaves_a_snapshot_snap_saved_lists` |
...
| E49 | Atlas carries a sideloaded snap when a run starts | The run finishes and leaves it installed and untouched on both machines | V | `test_package_sync:TestSideloadedSnapsThroughARealRun::test_a_sideloaded_snap_survives_a_whole_run_untouched` |
...
| E53 | A real run on VMs where one snap item fails | The run reports that item as failed and everything else it approved still landed | V | `test_package_sync:TestSnapPerItemFailureOnVMs::test_one_snap_item_fails_and_the_item_after_it_still_lands` |
...
| E113 | A real sync of two VMs with `snap_sync` and `folder_sync` both on | Nomad ends with no data directory for a revision its own snapd never installed | V | `test_package_sync:TestTheSnapDataBoundaryOnVMs::test_only_the_revision_the_target_holds_arrives_under_snap` |
| E115 | The user declines `alpha`'s install (or it fails), so Nomad's snapd never installs revision 10, and `folder_sync` runs afterwards | `~/snap/alpha/10` does not land on Nomad — the revision it holds data for was never installed there | U V | `test_snap_sync:TestExcludePaths::test_a_snap_the_target_does_not_hold_has_every_revision_dir_excluded`; `test_folder_sync:TestPackageJobExcludeFiltersGating::test_a_snap_the_target_does_not_hold_has_its_data_dir_excluded`; `test_package_sync:TestTheSnapDataBoundaryOnVMs::test_only_the_revision_the_target_holds_arrives_under_snap` |
...
| F72 | A remote is deleted | Its signing key goes with it, so nothing on Nomad still trusts that vendor | V | `test_package_sync:TestADeletedFlatpakRemoteTakesItsKey::test_deleting_an_unused_target_remote_removes_its_keyring_file` |
...
| G87 | A snippet that needs administrative rights it does not have on Nomad | Fails as its own item and is reported like any other; the job did not pre-check for it | U | `test_manual_installs_sync:TestContinueOnFailure::test_a_snippet_denied_administrative_rights_fails_like_any_other_item` |
...
| H175 | A question an article scopes to the work this run APPROVES (a repository deletion, a repository conflict, a collateral package a mark made in this review protects). | It is put in a second round built out of the first round's answers, and that round is a review like any other: its answers govern what applies. | U | `package_sync_core:TestTheSecondRoundIsPartOfTheReview::test_the_second_rounds_own_item_is_reviewed_and_converged` |
| H176 | That second round is reached. | Nothing has been changed on Nomad yet: it comes between the first round's answers and the job's first change, so a stopping answer still reaches every change it is about. | U | `package_sync_core:TestTheSecondRoundIsPartOfTheReview::test_nothing_is_converged_before_the_second_rounds_answers` |
| H177 | A run with no terminal whose only answerable question is the second round's. | The job is reported skipped, exactly as one whose first round held it — and the second round is never put to nobody. | U | `package_sync_core:TestTheSecondRoundIsPartOfTheReview::test_a_question_only_the_second_round_holds_still_skips_a_run_with_nobody_to_ask` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| J50 | `--dry-run` on a terminal | The same plan is built and the same review is put to the user as in a real run | P | `test_manual_installs_sync:TestClassificationAuthority::test_dry_run_previews_on_the_fly_install_without_replay_or_write` |
...
| K91 | A run of `flatpak_sync` | It transfers only signing keys and remote filter files — never `~/.var/app` or the flatpak store | P | `test_flatpak_sync:TestExcludePaths::test_returns_flatpak_data_dir_excludes_var_app` |
...
- **C176** — the one path that still reaches `AptSyncJob._withhold_repositories_still_in_use`. Off `_one_removal_context`, add a manual-collateral consequence of `vendor-tool`'s removal that only a mark made in the first round protects, approve the removal and the deletion, then keep the collateral package in the second round; assert no `rm` for `vendor.list` and the "approved deletion is not applied" line. Unit.
...
- **K91** — the cited test asserts which paths `folder_sync` excludes, which is a different claim from what a run of the job transfers. Assert the full `send_file` call list of a converging run. Unit.
...
- The snippet registry's exclusion from the `folder_sync` mirror: no integration run watches the registry stay put while the folder holding it is mirrored (K88).
