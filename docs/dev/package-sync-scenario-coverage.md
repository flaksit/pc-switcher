...
**Sections A–K default to one sync launched from Atlas** — that is how a row reads unless it says otherwise, and most do not. Two kinds say otherwise, and both say so in the row itself rather than leaving it to be inferred: about a dozen turn on what a *later* run does (B45, B47, C33, C61, C115, C119, D21, G35, G37, G82, G90, H114, H145 — "asked again next sync", "inert next run"), and four run with the roles swapped so Atlas is the machine being changed (A54, C175, H31, H126). A row that names no direction is the default.
