...
| C50 | the only packages coming from it are ones this run proposes to remove, and the user approves those removals | offered, alongside the removals — usage is counted after the removals the user APPROVED, so nothing on Nomad still installs from it by the time it goes | ‼ | `test_apt_job:TestRepoRemovalWithheldWhileInUse::test_a_repository_only_this_runs_removals_use_is_offered` asserts the offer, but `AptSyncJob._plan_repo_diffs` counts the removal CANDIDATES (its `going` set is built from the diffs, before any answer exists), so the offer is made on an approve-everything reading of a review that has not happened. Same defect as C172 |
...
| C81 | Nomad has the key with different bytes and Nomad's own dpkg owns that path | left alone only where the owner is Nomad's own distribution packaging; the repository is still written. Ownership by any other package is not an exemption (C174) | ‼ | `test_apt_keyrings:TestSharedKeyringsDirectory::test_a_package_owned_key_present_with_different_bytes_is_not_overwritten` asserts the exemption, but `AptProbe.capture_package_owned_keys` reads one batched `dpkg --search` and `AptKeyrings.manages` exempts every path it names, distribution or not |
...
| C174 | Nomad has the key with different bytes and the package owning that path is a vendor's, not the distribution's | refreshed from Atlas's copy like any other differing key — only the target's own distribution packaging is exempt | ‼ | none. `AptKeyrings.manages` exempts every path `dpkg --search` attributes to any package, so a vendor's key rotation never reaches Nomad. Same defect as C81 |
...
| E115 | The user declines `alpha`'s install (or it fails), so Nomad's snapd never installs revision 10, and `folder_sync` runs afterwards | `~/snap/alpha/10` does not land on Nomad — the revision it holds data for was never installed there | ‼ | none. `snap_sync_exclude_paths` enumerates Atlas's `~/snap` alone and always keeps the app's `current` revision, before and independently of the review, so the data travels whatever the answer was. K82 is the same defect for a declined revision CHANGE |
...
| F79 | Nomad's same-named remote has verification disabled while Atlas's has it on | The install is refused before it runs, naming both URLs as well as the two settings — every origin refusal names the two repositories the application would have come from | ‼ | `test_flatpak_sync:TestOriginIsReplicatedNotJustNamed::test_a_target_remote_that_does_not_verify_signatures_refuses_the_install` asserts the message the code builds, and `FlatpakSyncJob._origin_refusal`'s verification branch names the remote, the two settings and no URL at all (its URL branch names both) |
...
| G72 | Nomad holds the same item with the same body but a different label or authoring record | The user is asked and the entry is named: the transfer changes an entry Nomad holds, and the label and the authoring record are part of that entry | ‼ | none. `ManualInstallsSyncJob._guard_registry_overwrite` compares bodies alone, so every other field is overwritten unasked |
...
| H79 | Exemption: a log record. | It may use the role words in prose because it carries the machine it concerns as a field of its own. | — | the machine is carried as a field on every `_log`/`_announce` call, but the naming audit deliberately excludes the log, so nothing asserts it |
...
| K23 | `folder_sync` enabled; `snap_sync`/`flatpak_sync` toggled | Every package job behaves the same either way — the article binds package jobs to each other, and `folder_sync` is not one of them, so it is free to mirror `~/snap/<app>/<rev>` and `~/.local/share/flatpak` when their jobs are off and leave them alone when they are on | U | `unit/jobs/test_folder_sync:TestPackageJobExcludeFiltersGating::test_snap_sync_enabled_includes_revision_exclusion`, `::test_snap_sync_disabled_excludes_nothing` |
...
| N12 | Nomad has repository `S` and package `R` from it; Atlas has neither. Run 1 from Atlas, `R`'s removal declined | `S` is not raised at all — something on Nomad still uses it | ‼ | `test_apt_job`:`TestRepoRemovalWithheldWhileInUse::test_a_repository_only_this_runs_removals_use_is_offered` asserts the opposite (offered on a *proposed* removal) |
...
| N21 | Snap revisions converged in run 1, `folder_sync` running after | Nomad's `~/snap/<app>/<revision>` matches Atlas's, so the data lands where the target's snapd will read it | U | `test_folder_sync:TestPackageJobExcludeFiltersGating::test_snap_sync_enabled_includes_revision_exclusion` |
...
**C81, C174 — a vendor's key rotation is never followed.** `PKG-FR-KEY-REFRESH` exempts a differing key only where "the target's own **distribution** packaging owns it". `AptProbe.capture_package_owned_keys` parses `dpkg --search` and discards the package names, and `Keyrings.manages` then exempts any owned path, so no distribution-versus-vendor distinction exists anywhere. A key a third-party vendor's own package placed on Nomad is left stale, and the rotation the user has on Atlas never arrives.
...
**E115 — the same data-boundary defect as K82, reached by a declined install.** Where a snap's install is declined rather than its revision change, `folder_sync` still mirrors that snap's current-revision directory. One root cause — `snap_sync_exclude_paths()` reads Atlas's filesystem and takes no decisions and no outcomes — so one fix closes both and neither closes alone.

**F79 — the verification refusal names no URL.** `PKG-FR-FLATPAK-INSTALL-ORIGIN` says either origin failure must name both URLs. Only the URL-mismatch branch of `_origin_refusal` does; the branch for a remote that has stopped verifying signatures names the settings alone. That branch runs only once the URLs already matched, so what is missing is the one shared URL — the reader is told a setting differs without being told which repository it is about.

**F146 — a remote that stops verifying mid-run is invisible.** `_installed_origin_refusal` compares URL alone, so a verification setting that changes between the pre-install check and the read-back is never seen. It also resolves the landed origin against the cached `_target_remotes_now_by_id`, which nothing invalidates between the two, so the read-back re-reads the same snapshot: adding a verification comparison there would not close the window without a fresh read.
