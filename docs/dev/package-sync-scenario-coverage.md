...
| C175 | `apt.conf.d/20update` differs on the two machines and the user answers the overwrite permanently ("this one is this machine's own") | Nomad's copy is left exactly as it was; the mark is recorded on one machine, never synced; and the file is proposed in no later review, launched from either machine | — | none — no test covers the CHANGE direction of an apt-config mark (C124 covers the addition, C125 the deletion). Which machine holds it is also unsettled: `PackageSyncJob._decision_holder_is_source` records a change on Atlas, while the copy the answer protects is Nomad's |
