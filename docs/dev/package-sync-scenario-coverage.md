...
| A29 | `B→D→E→F`. Same branch as C22. | As C22. | U V | `test_apt_origins:TestOriginClassification::test_different_origin_install_derives_the_sources_own_repository`; `test_package_sync:TestTheAptOriginModelOnRealRepositories::test_a_vendor_repository_travels_with_the_package_and_the_install_lands_from_it` |
...
| E55 | Applying E54 | Nomad's `alpha` ends the run held | U V | same test (asserts `snap refresh --hold=forever alpha`); `test_package_sync:TestSnapHoldCaptureTiming::test_per_snap_hold_replicates_through_a_real_sync_window` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| F19 | A run that must add a remote Nomad lacks | No remote appears anywhere in the review; the remote is still written | U V | `test_flatpak_sync:TestRemotesAreDerivedFromApprovedRefs::test_no_remote_appears_in_any_review_group`, `TestMasksReachNoReviewGroup::test_ref_groups_keep_their_own_verbs_and_exclude_masks`; `test_package_sync:TestPackageSyncWholeRunContracts::test_flatpak_derives_the_remote_its_ref_needs_and_carries_its_key` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| F99 | Atlas restricts a remote with a filter file, and an approved application comes from that remote | The file is copied to the same absolute path on Nomad and the remote is set to use it there | U V | `test_flatpak_sync:TestRemoteFilterReplicates::test_the_file_is_copied_to_the_same_absolute_path_and_applied`; `test_package_sync:TestPackageSyncWholeRunContracts::test_a_source_filter_replicates_and_a_target_only_filter_comes_off` (run 1: same path, byte-identical content, filter recorded on Nomad) |
...
| G22 | Same branch as G88. | As G88. | U | `test_manual_installs_sync:TestInstallOnly::test_no_removal_diff_or_group_even_when_the_target_holds_items` |
...
| G28 | A stock Ubuntu 24.04 machine's own `/opt` and `/usr/local` are scanned | Nothing the distribution itself put there is presented: neither a scanned directory nor any other entry of `/usr/local`'s own skeleton | V | `test_package_sync:TestManualInstallsSyncEndToEnd::test_the_scan_of_a_real_machine_never_names_the_stock_skeleton` |
...
| G114 | Atlas's own `base-files` postinstall script is read | It still creates exactly the nine entries directly under `/usr/local` that the scan refuses to present | V | `test_package_sync:TestManualInstallsSyncEndToEnd::test_the_stock_skeleton_is_still_what_base_files_creates` |
| G112 | `dpkg` is missing on Nomad | Validation fails before anything runs, naming Nomad and the missing tool — the machine is read to tell what it already has | U | `test_manual_installs_sync:TestValidate::test_dpkg_unavailable_on_target_yields_validation_error` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| G88 | Nomad holds hand-installed software Atlas does not have | No removal is ever offered, in any list, and nothing only Nomad has is named anywhere | U | `test_manual_installs_sync:TestInstallOnly::test_no_removal_diff_or_group_even_when_the_target_holds_items` (the branch G22 names) |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| G109 | A finding Atlas has and Nomad already holds | It is not presented; Nomad is asked what it holds before anything is put to the user | U | `test_manual_installs_sync:TestWhatTheTargetAlreadyHolds::test_a_finding_the_target_already_holds_is_not_presented` |
...
| G111 | Atlas has a package no repository of its own supplies, and Nomad has that name installed from a repository | Not presented: software that is on Nomad is on Nomad, whatever origin put it there | U | `test_manual_installs_sync:TestWhatTheTargetAlreadyHolds::test_a_package_the_target_has_from_a_repository_counts_as_held` |
| G113 | Nothing on Atlas survives its own detection and marks | Nomad is not read at all — there is nothing to subtract from | U | `test_manual_installs_sync:TestWhatTheTargetAlreadyHolds::test_a_machine_with_no_findings_costs_the_other_one_no_reads` |
...
| H12 | Two package jobs are enabled and both machines are diverged. | Each manager's own approved item lands on Nomad's own package manager, so each reviewed before it mutated. | V | integration:`TestPackageSyncWholeRunContracts::test_one_failing_job_leaves_the_other_jobs_work_intact` |
...
| H69 | A report group's title names the machine that cannot reproduce an origin. | The hostname appears in the title. | U | `package_sync_core:TestReviewGroupsByAction::test_the_origin_report_title_names_the_machine_that_cannot_reproduce_them`, `::test_a_report_group_is_titled_by_its_cause_not_by_the_word_report` |
...
| H82 | A removal screen's answers are read. | The act is the concrete verb ("remove"); the permanent answer is "keep for good", not "always skip". | U | `package_review:TestInteractive::test_every_removal_direction_still_offers_the_permanent_answer` |
...
| H127 | A marked item whose identity exists on no input item (a digest-derived `apt.conf.d` file). | The post-diff pass drops it too, so it never comes back default-checked. | U | `block_state_decisions:TestAptRepoItemDecisions::test_declined_config_install_is_recorded_on_source_and_never_re_offered` |
| H128 | A decision file already holds an entry naming a block — a hold, a mask. | It silences nothing: the block still replicates, and so does the software it applies to. | U | `block_state_decisions:TestABlockCanNeverBeRecorded::test_a_recorded_snap_hold_decision_does_not_silence_the_replication`, `::test_a_recorded_apt_hold_decision_does_not_silence_the_replication` |
...
| H173 | The variable is set on a run that DOES have a terminal. | It still answers the review silently, unprompted — the accepted cost. | U | `package_review:TestAutomationEnv::test_the_variable_answers_a_review_on_a_terminal_too`, `::test_automation_env_returns_mapped_decisions_without_prompting` |
...
| K20 | Two package jobs enabled, both machines diverged for both | Each manager settles its own review before that same manager changes Nomad; neither waits on the other's review | V | `integration/jobs/test_package_sync:TestPackageSyncWholeRunContracts::test_one_failing_job_leaves_the_other_jobs_work_intact` |
...
| N22 | Two machines, all four package jobs enabled, a full run | every job reviews before its own first change, one job's failure leaves the others' work intact, and the exit code reflects what failed | V | `test_package_sync`:`TestPackageSyncWholeRunContracts::test_one_failing_job_leaves_the_other_jobs_work_intact` (all four enabled, the job ordered first fails, apt's and snap's approved work still lands on Nomad, the run exits non-zero) |
