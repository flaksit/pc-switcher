...
| `test_snap_sync`, `test_flatpak_sync`, `test_manual_installs_sync`, `test_folder_sync` | `tests/unit/jobs/` |
...
| `test_package_sync` | `tests/integration/jobs/test_package_sync.py` — the only source of a `V` |

A `V` is a claim about a test that runs only on two provisioned VMs. The integration suite last passed whole at `a2c299dc`, on the pull request for this branch; a `V` added after that commit has not yet run.
...
| A29 | `B→D→E→F`. Same branch as C22. | As C22. | U | `test_apt_origins:TestOriginClassification::test_different_origin_install_derives_the_sources_own_repository` |
...
| A65 | Same branch as C2. | As C2. | V | `test_package_sync:TestAptSyncEndToEnd::test_apt_repository_state_dry_run_previews_derived_writes_and_reviews_no_repository` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B1 | Atlas holds `pkg-a`; Nomad has `pkg-a` at the same version and does not hold it, so `pkg-a` itself is no item this run | One item, `pkg-a (hold)`, in a group whose verb is "hold"; approving it registers the hold on Nomad and issues no install or upgrade | U | `test_apt_packages:TestAptHold::test_source_held_yields_install_hold_item_and_converge_runs_apt_mark_hold`; `test_apt_diffing:TestDiffEngine::test_source_hold_only_yields_apt_hold_install` |
| B2 | Nomad holds `pkg-a` (and has it); Atlas has `pkg-a` and does not hold it, so `pkg-a` itself is no item this run | One item, `pkg-a (hold)`, in an "unhold" group; approving it removes the hold from Nomad | U | `test_apt_packages:TestAptHold::test_target_held_only_yields_remove_unhold_item`; `test_apt_diffing:TestDiffEngine::test_target_hold_only_yields_apt_hold_remove_and_suppresses_package_action` |
...
| B5 | One run carrying an install, a removal, a hold add and a hold removal, where the two holds name packages that are not themselves items this run | The two hold items sit in their own groups reading "hold"/"unhold"; the package items keep "install"/"remove", and no hold appears under a package group | U | `test_apt_job:TestHoldReviewVerbs::test_hold_items_get_their_own_group_with_hold_and_unhold_verbs` |
...
| B7 | Atlas holds `pkg-a`, Nomad lacks it | There is no way to approve the install and decline the hold: one row, whose own text says Nomad ends up holding it, and one answer that installs at Atlas's version and then holds | U | `test_apt_packages:TestAHeldPackageIsInstalledAtTheSourcesVersion::test_the_install_and_its_hold_are_one_question_and_one_answer` |
...
| B12 | Same branch as B3. | As B3. | U | `test_apt_packages:TestAStaleTargetHoldDoesNotStrandThePackage::test_a_hold_on_a_package_the_target_has_still_suppresses_its_install` |
...
| B14 | Nomad has and holds `pkg-a`; Atlas does not have it at all | No removal item for `pkg-a` (it produces no package-level item in any direction); the unhold is offered | U | `test_apt_diffing:TestDiffEngine::test_a_package_only_the_target_has_and_holds_yields_the_unhold_alone` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B16 | Nomad records a hold for `pkg-a` but does not have `pkg-a`; Atlas has it and holds it | `pkg-a` is offered for install like any other package, and Atlas's hold rides that one question and lands after the install | U | `test_apt_diffing:TestDiffEngine::test_a_hold_for_a_package_the_target_lacks_still_proposes_the_install` |
| B17 | Same, but Atlas does not hold `pkg-a` | Two changes, one question: `pkg-a` is offered for install and Nomad's own bookkeeping hold comes off with it | U | `test_apt_diffing:TestDiffEngine::test_a_stale_hold_the_source_does_not_share_proposes_install_and_unhold` |
| B18 | Nomad records a hold for `pkg-a`; neither machine has `pkg-a` installed | No package item; the hold is still offered for removal (Nomad carries selection state Atlas does not) | U | `test_apt_diffing:TestDiffEngine::test_a_hold_for_a_package_neither_machine_has_yields_the_unhold_alone` |
| B19 | The one question of B16 approved, run through | Nomad's bookkeeping hold is cleared, then the package is installed, then the hold Atlas asked for is registered — in that order | U | `test_apt_packages:TestAStaleTargetHoldDoesNotStrandThePackage::test_the_stale_hold_is_cleared_the_package_installed_and_the_hold_restored` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B31 | Atlas holds `pkg-a`, Nomad lacks it; the one question covering both is declined for this run | Neither half runs: no install, no `apt-mark hold`, and the hold is logged as skipped this run rather than as a failure | U | `test_apt_packages:TestAHoldNeedsItsPackage::test_a_hold_whose_install_was_skipped_is_declined_not_failed` |
| B32 | Same, but the answer stood and the install is withdrawn by a collateral answer given while planning (the user kept a package on Nomad that installing `pkg-a` would have removed) | The hold follows the rewritten answer: declined, no failure, nothing logged as an error | U | `test_apt_packages:TestAHoldNeedsItsPackage::test_a_hold_whose_install_a_collateral_answer_cancelled_is_declined_too` |
...
| B35 | Atlas holds `pkg-a` and gets it from a repository this run cannot reproduce, so `pkg-a` is reported rather than installed and is nothing the user answers; the hold is therefore its own item, and is approved | The hold fails alone, saying `pkg-a` is not on Nomad and the run cannot reproduce the repository it comes from | U | `test_apt_packages:TestAHoldNeedsItsPackage::test_a_hold_on_a_package_no_repository_can_supply_fails_alone` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B38 | `pkg-a` missing on Nomad and held on Atlas; the one question covering both approved; the run has a repository item that is left unapproved | The install runs before the hold | U | `test_apt_packages:TestInstallBeforeHoldOrdering::test_hold_follows_install_on_the_plain_plan_sort_path` |
...
| B41 | Atlas holds `pkg-a` but does not have it in its manual set, while Nomad has `pkg-a` manually | One question, in the removal group, whose own text says Nomad is left holding nothing for it; approving it removes `pkg-a` from Nomad, runs no `apt-mark hold`, and reports the hold as not applied rather than as a failure | U | `test_apt_packages:TestAHoldWhosePackageIsBeingRemoved::test_the_removal_and_the_hold_are_one_question`, `::test_approving_it_removes_the_package_and_registers_no_hold` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B42 | The hold add for `pkg-a` is its own item and is marked machine-specific | The mark is written on Atlas (the machine that holds the hold), not on Nomad, and `pkg-a`'s hold is never offered again | U V | `test_block_state_decisions:TestAptHoldDecisions::test_declined_hold_is_recorded_on_source_and_never_re_offered`; `test_package_sync:TestBlockStateDecisionRoundTrip::test_skip_always_on_an_apt_hold_is_inert_next_run` |
| B43 | The unhold for `pkg-a` (a real hold on Nomad) is its own item and is marked machine-specific | The mark is written on Nomad, not on Atlas, and the unhold is never offered again | U | `test_block_state_decisions:TestAptHoldDecisions::test_declined_unhold_is_recorded_on_target_and_never_re_offered` |
...
### B.10 The hold rides its package where the package is itself an item (articles: PKG-FR-BLOCKS-REPLICATE, PKG-FR-APT-HOLD-ITEM's exception)

| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B52 | Atlas has `pkg-a` and holds it; Nomad has neither | One row, `pkg-a`, whose own text says Nomad ends up holding it as Atlas does; `pkg-a (hold)` appears in no group; approving that row installs at Atlas's version and then holds | U | `test_apt_packages:TestAHeldPackageIsInstalledAtTheSourcesVersion::test_the_install_and_its_hold_are_one_question_and_one_answer` |
| B53 | Atlas has and holds `pkg-a`, Nomad has it at the same version — the package is no item this run | The hold keeps a row of its own, in its own "Hold apt packages" group, and that row carries no rider text | U | `test_apt_packages:TestAptHold::test_a_hold_whose_package_is_no_item_this_run_keeps_a_row_of_its_own` |
| B54 | The one question of B52 answered "skip this run" | Neither half runs, and neither machine's decision file is written — the pair is offered together again on the next sync | U | `test_apt_packages:TestAHeldPackageIsInstalledAtTheSourcesVersion::test_declining_the_one_question_leaves_neither_the_package_nor_the_hold` |
| B55 | The one question of B41 answered permanently | Two marks from one answer, each on its own holding machine: `apt:package:pkg-a` on Nomad, which has the package, and `apt:hold:pkg-a` on Atlas, which holds the hold. The next run raises neither | U | `test_block_state_decisions:TestAMarkGivenOnAMergedQuestion::test_one_permanent_answer_writes_a_mark_on_each_holding_machine` |
...
| C10 | `/etc/apt/sources.list` on Nomad and not on Atlas | never offered for deletion | U | `test_apt_probe:TestWhatAptItselfReads::test_sources_list_is_digested_on_both_machines_and_is_still_not_an_item` |
...
| C172 | the only package coming from `vendor.list` is one this run proposes to remove, and the user declines that removal | `vendor-tool` stays on Nomad, so something still uses the repository: it is not raised as an item at all and it is not deleted | ‼ | `test_apt_job:TestRepoRemovalWithheldWhileInUse::test_a_repository_whose_packages_removal_was_declined_is_not_deleted` proves it is not deleted; it IS raised, because both answers are given in one review and the row's gate is one of them |
...
| C173 | the key a repository this run writes names is one an Atlas package owns — a vendor `.deb` that shipped both the repository entry and its keyring — and Nomad lacks it | copied byte-for-byte all the same: what owns a key on Atlas never gates the copy | — | none. The run asks no machine but Nomad what owns a key (`dpkg --search` runs only in `AptProbe.capture_package_owned_keys`, on the target), so the branch holds by construction |
...
| C175 | `apt.conf.d/20update` differs on the two machines and the user answers the overwrite permanently ("this one is this machine's own") | Nomad's copy is left exactly as it was; the mark is never synced; and the file is proposed in no later review, whichever machine that run is launched from. Which machine records the mark is unsettled — see the Test column | — | none — no test covers the CHANGE direction of an apt-config mark (C124 covers the addition, C125 the deletion). Which machine holds it is also unsettled: `PackageSyncJob._decision_holder_is_source` records a change on Atlas, while the copy the answer protects is Nomad's |
...
| D20 | The go-ahead answer is read | It states its own effect, naming the causing change and the consequence: "install sl on nomad, so fortunes is removed as well"; the word on the row is the effect verb (`remove`/`downgrade`/`upgrade`) | U | `test_apt_collateral:TestTheAnswersTheItemComposes::test_an_install_cause_phrases_both_answers_around_the_install` |
...
| D37 | The user SKIPPED `pkg-b`'s removal for this run; the approved removal of `pkg-a` would carry `pkg-b` off anyway | The requirement says a skipped candidate keeps its protection, so the user must be ASKED. The tool tells instead: `plan_time` exempts every removal candidate from the removal batch, so no question is built, and `PackageConverger.remove`'s guard refuses `pkg-a`'s transaction naming `pkg-b`. Nothing is lost; the answer is never offered. Accepted cost: closing it needs one `apt-get --dry-run` per candidate on every run with removals | ‼ | the accepted cost the criteria record. Pinned as it stands by apt/collateral:`TestASkippedRemovalKeepsItsProtectionAtTheGuard::test_a_skipped_candidate_is_refused_by_name_rather_than_asked_about` — the day the gap closes, that test flips from refused to asked |
| D38 | Nomad holds `pkg-a` without having it installed, and `pkg-a` is in this run's install batch | `Collateral.plan_time` rehearses with `--allow-change-held-packages`, which the real install withholds, so apt may report changes to OTHER held packages that the real command would refuse outright — the question can be about collateral that would never happen. Accepted: over-asks, never under-asks | ‼ | the accepted cost the criteria record. apt/collateral:`TestTheRehearsalSurvivesAStaleTargetHold` (3) pins both the flag and the over-ask it can produce |

#### The guard behind the question

| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| D39 | The real install's transaction drifts after plan time and would remove a manually-installed package nobody saw | The user is asked before that transaction runs, with the three answers — go ahead, keep the package, stop the sync — because the fact only exists once the run's own changes landed; nothing is removed on an answer nobody gave | ‼ | apt/packages:`TestTransactionGuard::test_guard_refuses_drifted_manual_removal_not_seen_at_plan_time` asserts what the code does instead: `PackageConverger.install` refuses the transaction as a failed item naming the package, so the user is told rather than asked |
| D40 | The real removal's transaction drifts and would remove a manually-installed package nobody saw | Same three-way question, before the removal runs | ‼ | apt/packages:`TestRemovalGuard::test_drifted_manual_reverse_dep_removal_refused` asserts the refusal `PackageConverger.remove` raises instead |
| D41 | The real transaction would downgrade a manually-installed package nobody saw | Same three-way question, before the transaction runs | ‼ | apt/packages:`TestDowngradeGuard::test_guard_refuses_drifted_manual_downgrade` asserts the refusal instead |
| D71 | The real transaction would UPGRADE a manually-installed package nobody saw at plan time | Same three-way question, before the transaction runs — an unasked-for upgrade is the imposition a downgrade is | ‼ | none. `Collateral.unapproved` classifies the drifted upgrade exactly as `classify` does at plan time, and the converge guard then refuses the transaction; no test drives an apply-time upgrade drift at all |
...
| D57 | A consequence already let go ahead at plan time comes up again in the late (post-`/etc/apt`) question | It is not asked twice — the id is the consequence, so the earlier answer covers this cause | U | `test_apt_collateral:TestAConsequenceAlreadyAnsweredAtPlanTime::test_a_go_ahead_given_at_plan_time_is_not_asked_for_again` |
...
| E19 | A plan carrying a revision/channel change is reviewed | The change group reads as a change, distinct from the install and remove groups | U | `test_snap_sync:TestAFullSnapReview::test_the_change_group_reads_as_a_change_and_stands_apart` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| E36 | An approved removal of a snap from Nomad | The snap is removed and snapd's own pre-removal snapshot is left in place | U | `test_snap_sync:TestConvergeRemoval::test_removal_never_passes_purge` |
...
| E49 | Atlas carries a sideloaded snap when a run starts | The run finishes and leaves it installed and untouched on both machines | — | no VM coverage of sideloads |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| E54 | Atlas holds `alpha`; Nomad has `alpha` at the same revision and channel and does not hold it, so `alpha` itself is no item this run | A hold item of its own, separate from `alpha`, proposing to add the hold on Nomad | U | `test_snap_sync:TestHolds::test_source_held_yields_install_hold_diff_and_converges_hold_forever` |
...
| E61 | A plan carrying snap installs, snap removals and holds in both directions, where the holds name snaps that are not themselves items this run | Hold items sit in their own groups reading "hold"/"unhold", never inside the install or remove groups | U | `test_snap_sync:TestHoldReviewVerbs::test_hold_install_group_reads_hold_never_install`, `::test_hold_remove_group_reads_unhold_and_is_removal_direction`, `::test_snap_groups_keep_their_own_verbs_and_exclude_hold_items` |
...
| E63 | `alpha` is new on Nomad AND held on Atlas | One question settles both, and the install is applied before the hold, so the hold lands on a snap that exists | U | `test_snap_sync:TestHolds::test_hold_diff_emitted_after_presence_diffs` |
| E64 | `alpha`'s one question is approved and the install then fails | The hold fails as its own item; a hold whose snap is no item this run still lands | U | `test_snap_sync:TestHoldAndRevisionFailuresArePerItem::test_hold_for_a_snap_absent_on_target_fails_only_that_item` |
...
| E68 | The user answers "never ask again" to a hold Atlas holds, decided on its own | The mark is recorded on Atlas, and the hold is never offered again | U V | `test_block_state_decisions:TestSnapHoldDecisions::test_declined_hold_is_recorded_on_source_and_never_re_offered`; `test_package_sync:TestBlockStateDecisionRoundTrip::test_skip_always_on_a_snap_hold_is_inert_next_run` |
...
| E70 | A hold decided on its own was permanently declined; a later run finds `alpha` gone from Nomad | `alpha` is offered for install and that question carries no hold text — the mark silences the hold, not the snap | U | `test_block_state_decisions:TestSnapHoldDecisions::test_recorded_hold_does_not_silence_the_snaps_own_presence_diff` |
...
| E116 | Atlas holds `alpha` and Nomad does not have it at all | One row, `alpha`, whose own text says Nomad ends up holding its refreshes as Atlas does; `alpha (hold)` appears in no group; approving it installs the snap and then holds it | U | `test_snap_sync:TestHolds::test_a_hold_on_a_snap_this_run_installs_is_one_question_with_it` |
...
| E88 | The suspension is written | It is a timed value computed on that machine's own clock, so it lapses on its own | U | `test_snap_autorefresh_hold:TestHoldEngaged::test_hold_set_on_both_hosts_when_snap_sync_enabled` |
...
| E115 | The user declines `alpha`'s install (or it fails), so Nomad's snapd never installs revision 10, and `folder_sync` runs afterwards | `~/snap/alpha/10` does not land on Nomad — the revision it holds data for was never installed there | U | `test_snap_sync:TestExcludePaths::test_a_snap_the_target_does_not_hold_has_every_revision_dir_excluded`; `test_folder_sync:TestPackageJobExcludeFiltersGating::test_a_snap_the_target_does_not_hold_has_its_data_dir_excluded` |
...
| F43 | ‼ Atlas verifies a remote whose key it holds through the ostree per-remote option `gpgkeypath` | Required: the key travels. Actual: `_stage_source_keys` reads only `<remote>.trustedkeys.gpg` and the machine-level anchor directory, so the remote is replicated as holding no key at all and installs from it fail the signature check on Nomad. Accepted cost, recorded in the criteria: flatpak never writes that option and only a hand-edited repo config can set it | ‼ | `test_flatpak_sync:TestRemoteTrustTravelsWithTheDerivedWrite::test_a_key_held_only_through_gpgkeypath_is_replicated_as_no_key_at_all` pins the accepted behaviour: nothing reads the repo config the option lives in, so the remote is added plainly and warned about |
...
| F103 | Atlas no longer restricts a remote Nomad still restricts, and an approved application comes from it | Nomad ends the run unfiltered on that remote; nothing is re-applied | U V | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_filter_the_source_no_longer_has_is_taken_off_the_target`; `test_package_sync:::test_a_source_filter_replicates_and_a_target_only_filter_comes_off` (run 2) |
...
| F110 | flatpak refuses to re-apply the filter | Every approved application from that remote fails, quoting flatpak's error and naming the remote and path | U | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_filter_flatpak_refuses_fails_the_app_and_quotes_the_error` |
...
| F116 | The remote is system-scope, so both the filter write and the re-apply need root | The writes run under sudo and a user-scope one never escalates | U | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_system_scope_filter_writes_and_applies_under_sudo`, `test_flatpak_sync:TestRemoteFilterReplicates::test_a_user_scope_filter_never_escalates` |
| F117 | ‼ The clear succeeds, the applications install, and the re-apply then fails | Required: the applications fail saying so — which they do. Accepted cost, recorded in the criteria: until the next run Nomad's remote offers more than either machine meant | ‼ | `test_flatpak_sync:TestRemoteFilterReplicates::test_a_re_apply_that_fails_leaves_the_remote_unfiltered_for_the_rest_of_the_run` |
...
| F129 | A run installing one application and removing another, carrying a mask whose pattern covers neither | The mask has its own item and its own group; neither application group contains a mask | U | `test_flatpak_sync:TestMaskReviewVerbs::test_ref_groups_keep_their_own_verbs_and_exclude_masks` |
...
| F147 | Nomad masks `org.example.Blocked` in the system scope, Atlas masks nothing, and the user approves the unmask | The pattern is gone from Nomad's mask list afterwards — `flatpak --system mask` no longer prints it | U | `test_flatpak_sync:TestMaskConverge::test_system_scope_mask_removal_uses_sudo_and_remove_flag` |
| F148 | Atlas masks `org.example.App` in the user scope and has `org.example.App/x86_64/stable` there; Nomad has neither | One row, the application's, whose own text says Nomad ends up masking `org.example.App`; the mask appears in no group | U | `test_flatpak_sync:TestAMaskRidesTheApplicationItCovers::test_a_mask_covering_an_installed_application_is_one_question_with_it` |
| F149 | Atlas masks `org.example.*` and has two applications this run would install under that pattern | The mask keeps a row of its own: two answers can disagree, so there is no single decision for it to obey. Neither application's row mentions it | U | `test_flatpak_sync:TestAMaskRidesTheApplicationItCovers::test_a_pattern_covering_two_of_this_runs_items_keeps_its_own_question` |
| F150 | Atlas masks `org.example.App` and does not have it; Nomad has `org.example.App/x86_64/stable` | One question: approving it uninstalls the application and then applies the mask. Unlike a hold, a mask riding a removal still lands | U | `test_flatpak_sync:TestAMaskRidesTheApplicationItCovers::test_a_mask_riding_a_removal_still_lands` |
...
| G5 | Atlas has a hand-installed `.deb` that apt marks automatically-installed (absent from `apt-mark showmanual`) | Presented as an item Nomad cannot get from any package manager: what the job covers is every installed version no configured repository supplies, and `apt-mark showmanual` is not the boundary the article draws | U | `test_manual_installs_sync:TestNoCandidateDetection::test_a_hand_deb_apt_marks_automatic_is_still_detected` |
| G6 | Atlas has a package hand-installed at a version NEWER than any repository offers: its `***` row has no repository origin while older rows do | Presented — the repository cannot supply the version this machine has | U | `test_manual_installs_sync:TestNoCandidateDetection::test_a_version_newer_than_any_repository_offers_is_unreproducible` |
...
| G8 | apt answers about the installed set but says nothing about one queried name | That name is not presented; its silence indicts nothing | U | `test_manual_installs_sync:TestNoCandidateDetection::test_no_block_inside_an_answered_policy_read_indicts_nothing` |
...
| G11 | Atlas's entire installed set was hand-installed from `.deb` files, so apt reports every one of them with no repository | An ordinary answer: every one is presented | U | `test_manual_installs_sync:TestNoCandidateDetection::test_a_policy_read_over_only_bare_deb_packages_still_answers` |
| G12 | Reading Atlas's installed package set fails | The job fails once naming that read; the run continues with the other jobs | U | `test_manual_installs_sync:TestNoCandidateDetection::test_an_installed_set_read_that_did_not_answer_fails_the_job` |
...
| G22 | Same branch as G88 and G89. | As G88 and G89. | U | `test_manual_installs_sync:TestInstallOnly::test_no_removal_diff_or_group_even_when_the_target_holds_items` |
...
| G35 | An item answered "not for now" this run | Nothing is written anywhere, so the next sync asks about it again | U | `test_manual_installs_sync:TestPermanentMarkWrites::test_not_for_now_records_nothing_on_either_machine` |
| G36 | An item answered "never install it on Nomad" | The mark is written on Atlas, the machine that holds the software — not on Nomad | U | `test_manual_installs_sync:TestPermanentMarkWrites::test_never_install_it_records_the_mark_on_the_source_naming_the_item` |
...
| G45 | Nomad holds a permanent mark for the item; Atlas holds none | The item is still presented — only Atlas's marks silence an Atlas-held finding | U | `test_manual_installs_sync:TestInertFiltering::test_a_mark_on_the_target_does_not_silence_a_source_held_finding` |
...
| G52 | A run that writes one snippet and then continues to the install stage | The snippet's record is stamped once, and Atlas's and Nomad's copies of the registry are identical afterwards | U | `test_manual_installs_sync:TestSnippetPush::test_a_snippet_written_this_run_is_stamped_exactly_once` |
...
| G54 | A rehearsal where Atlas already holds a snippet for a finding | Previewed as an install on Nomad, naming the item; nothing runs there | U | `test_manual_installs_sync:TestClassificationAuthority::test_dry_run_previews_a_pre_existing_snippet_install_naming_the_item` |
| G55 | A rehearsal where a finding is answered "never install it on Nomad" | No mark is written on Atlas | U | `test_manual_installs_sync:TestPermanentMarkWrites::test_a_rehearsal_records_no_permanent_mark` |
...
| G72 | Nomad holds the same item with the same body but a different label or authoring record | The user is asked and the entry is named: the transfer changes an entry Nomad holds, and the label and the authoring record are part of that entry | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_a_different_authoring_record_is_non_additive_and_prompts`, `::test_a_different_label_is_non_additive_and_prompts` |
...
| G74 | The user declines, and other jobs were still to run | The run ends there rather than continuing with the remaining jobs | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_lost_target_entry_aborts_on_decline` |
...
| G78 | A snippet label or body containing square-bracketed text reaches that question | Shown as written; the display does not break | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_a_bracketed_label_and_body_reach_the_question_as_written` |
| G79 | Nomad's registry file is corrupt | The sync ends there: nothing is asked, nothing is sent, and Nomad is never read as holding nothing | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_a_corrupt_target_registry_ends_the_run_and_sends_nothing` |
| G80 | Atlas's registry file on disk is corrupt at transfer time | The sync ends there too — no question, and the corrupt bytes never reach Nomad | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_a_corrupt_source_registry_ends_the_run_and_sends_nothing` |
...
| G83 | A run at a terminal whose scan found nothing to review | The registry still travels: an empty review means nothing new to decide, not nothing to carry | U | `test_manual_installs_sync:TestSnippetPush::test_an_answered_run_with_nothing_to_review_still_transfers_the_registry` |
| G84 | Creating the destination directory on Nomad fails, or Nomad's home cannot be resolved | The job fails naming Nomad; nothing is half-transferred | U | `test_manual_installs_sync:TestSnippetPush::test_a_directory_that_cannot_be_created_fails_naming_the_target_and_sends_nothing`, `test_manual_installs_sync:TestSnippetPush::test_a_home_that_cannot_be_resolved_fails_naming_the_target_and_sends_nothing` |
...
| G87 | A snippet that needs administrative rights it does not have on Nomad | Fails as its own item and is reported like any other; the job did not pre-check for it | U | `test_manual_installs_sync:TestContinueOnFailure::test_failed_snippet_replay_is_a_per_item_failure_and_does_not_stop_the_job` |

### G.7 What is never done (articles: PKG-NG-MANUAL-REMOVE, PKG-FR-JOB-INDEPENDENCE for this job)

| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| G88 | Nomad holds hand-installed software Atlas does not have | No removal is ever offered, in any list | U | `test_manual_installs_sync:TestInstallOnly::test_no_removal_diff_or_group_even_when_the_target_holds_items` (also half the branch G22 names) |
| G89 | Nomad is asked what unreproducible software it holds | It is never asked — there is no such question to put to it | U | `test_manual_installs_sync:TestInstallOnly::test_target_query_is_empty_by_design` (also half the branch G22 names) |
| G90 | A run replays two snippets successfully on Nomad | Nothing on Nomad records what this job put there; a later run has no memory of it | U | `test_manual_installs_sync:TestSnippetPush::test_a_successful_replay_records_nothing_on_the_target` |
...
| H35 | Several groups are reviewed in one round. | The screens come one after another with no executor command between them. | U | `package_sync_core:TestNoWorkBetweenTheQuestionsOfOneRound::test_no_executor_command_is_issued_between_the_screens_of_one_round` |
...
| H79 | Exemption: a log record. | It may use the role words in prose because it carries the machine it concerns as a field of its own. | U | `package_sync_core:TestEveryLogRecordCarriesItsMachine::test_no_package_job_log_record_leaves_its_machine_unsaid` |
...
| H101 | An unhold or unmask (removal-direction block state) screen is confirmed unread. | Nothing is unheld or unmasked. | U | `test_review_skip_always:TestThePermanentAnswer::test_every_promotable_direction_offers_all_three_answers`, `test_review_skip_always:TestBlockStateItemsArePromotable::test_an_unhold_or_unmask_screen_opens_on_keeping_the_block` |
...
| H109 | A collateral screen opens. | It starts at "keep the package", not at "go ahead". | U | `test_package_review:TestCollateralGroupResolution::test_the_row_starts_at_keeping_the_package` |
...
| H123 | A flatpak mask add / unmask is marked. | Same, in the flatpak decision file. | U | `test_block_state_decisions:TestFlatpakMaskDecisions::test_declined_mask_is_recorded_on_source_and_never_re_offered`, `::test_declined_unmask_is_recorded_on_target_and_never_re_offered` |
...
| H133 | A third machine, Vega, is synced for the first time. | It holds no marks; every item is decided again there. | P | H130 read with H131 and H147: no decision file travels by either route that carries files between machines, and a machine that has never been synced therefore has none — which reads as "no permanent decisions", so nothing filters its items |
...
| H167 | The map names a permanent decision for an unreproducible item. | The machine-specific mark is written and no snippet is: authoring one takes an editor, which the map has no way to stand in for. | — | nothing asserts the absence; `_decisions_from_automation` returns decisions alone, so `ReviewOutcome.snippets` is empty on that path by construction |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| J50 | `--dry-run` on a terminal | The same plan is built and the same review is put to the user as in a real run | U | `test_manual_installs_sync:TestClassificationAuthority::test_dry_run_previews_on_the_fly_install_without_replay_or_write` |
...
| J60 | `--dry-run` with no terminal and a review holding something to decide | The job reports skipped, for the same reason a real run does | U | `package_sync_core:TestExecuteSelfContained::test_a_rehearsal_with_no_terminal_skips_the_job_for_the_same_reason` |
...
| J83 | The `dpkg-query` naming Atlas's installed set for the detection scan does not answer | The job fails | U | `test_manual_installs_sync:TestNoCandidateDetection::test_an_installed_set_read_that_did_not_answer_fails_the_job` |
...
| J106 | The same automatic collateral at apply time, from the transaction that actually happens | Logged there as well as at plan time | — | `Collateral.unapproved` calls `_log_auto` and the apt converge tests reach it, but no test asserts the line it writes |
| J107 | A manager makes a self-directed change other than apt's dependency resolution | Logged the same way | P | no second case exists to test: apt's dependency resolution is the only self-directed change any of the four managers makes (ADR-021), so J104–J106 are the whole of this branch as it can be reached today. What the row still fixes is that a second one would be logged the same way, which is unprovable until there is one |
...
| J122 | A review line whose label or detail contains a credentialed URL | Shown redacted | U | `test_redaction:TestItemDiffText::test_every_string_the_user_reads_while_deciding_is_redacted` |
...
| J127 | The label a permanent decision keeps on disk | Written redacted | U | `test_redaction:TestItemDiffText::test_every_string_the_user_reads_while_deciding_is_redacted` |
...
| J145 | A whole sync | What software Atlas has, and where it gets it from, is unchanged | U | `test_package_sync_core:TestIdempotency::test_identical_source_and_target_produce_no_diff_no_group_and_no_mutation` |
...
| J164 | The prompt has no default | An accidental Enter re-prompts rather than choosing | U | `test_step_gate:TestTerminalUIStepGate::test_the_prompt_offers_no_default` |
...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| K15 | `apt_sync` on, `manual_installs_sync` off; Atlas holds a package installed from a hand-downloaded `.deb` | `apt_sync` produces no item, no review line and no install for it — the same as when `manual_installs_sync` is on | U | `test_apt_probe:TestBareDebPackagesAreNotAptSyncsBusiness::test_bare_deb_package_produces_no_diff_and_no_review_entry` |
...
| K43 | `apt_sync` enabled; `apt-mark` missing on Atlas | Validation fails naming Atlas | U | `test_apt_job:TestValidate::test_apt_mark_unavailable_yields_validation_error` |
...
| K62 | `manual_installs_sync` enabled; neither machine grants passwordless sudo | Validation passes — this job requires none on either machine | U | `test_manual_installs_sync:TestValidate::test_valid_environment_yields_no_errors` |
...
| K67 | A precondition is missing | It is discovered in the validation step, never mid-execute: no job re-probes sudo or the dpkg lock while applying, and none degrades to a reduced capture | P | `test_mutates_audit:TestPreconditionProbes::test_no_precondition_is_probed_outside_validate` resolves every executor command back to its literals and finds `sudo --non-interactive` and the `fuser` lock probe in three `validate()` bodies and nowhere else, `::test_the_audit_can_read_the_commands_it_scans` guards the resolution; nothing asserts the second half — that no job degrades to a reduced capture rather than refusing |
...
| K88 | `manual_installs_sync` skipped (no terminal) or declined, `folder_sync` enabled | Nomad's snippet registry is not overwritten by this run | U | `unit/jobs/test_folder_sync:TestSnippetRegistryExcludeFilters::test_the_registry_is_excluded_under_the_invoking_users_home`, `::test_excluded_even_with_every_package_job_off`, `::test_the_registry_exclusion_precedes_the_merge_filter`, `::test_registry_outside_synced_folder_is_skipped` |
| K89 | `apt_sync` enabled with `folder_sync` | No `folder_sync` exclusion is needed for apt: everything `apt_sync` writes lives outside the synced folders (`/etc/apt`, the dpkg database) | P | `test_mutates_audit:TestTransferDestinations::test_every_package_transfer_lands_where_the_table_says` resolves each package transfer's destination back to the literals it is built from, `::test_no_package_transfer_can_land_outside_the_staging_and_config_paths` requires each to be a staging directory or the snippet registry, and `::test_no_package_job_names_a_manager_store_it_has_not_accounted_for` requires every mention of `/var/lib/dpkg`, `/var/lib/snapd`, `/var/lib/flatpak` or `/var/snap` to be named. Unasserted: apt also writes a transient staging copy under `~/.cache/pc-switcher/apt-staging`, which IS inside the synced folder — removed in a `finally` and already covered by `home.filter`'s `- .cache/*`, so no apt-specific rule is needed, but nothing asserts either fact |
...
| K91 | A run of `flatpak_sync` | It transfers only signing keys and remote filter files — never `~/.var/app` or the flatpak store | U | `test_flatpak_sync:TestExcludePaths::test_returns_flatpak_data_dir_excludes_var_app` |
...
| N2 | Same mark, then run 3 from Nomad — roles reversed, so `wireshark` is now target-only | still no item: the mark protects it from removal by a sync from the other machine, not only from being installed | V | `test_package_sync`:`TestPackageSyncWholeRunContracts::test_skip_always_is_inert_in_both_roles` (run 3, pc2→pc1) |
...
| N4 | An apt hold decided on its own — its package no item that run — marked machine-specific in run 1 | run 2 raises no hold item, and the package's own item is unaffected | U V | `test_block_state_decisions`:`TestAptHoldDecisions::test_declined_hold_is_recorded_on_source_and_never_re_offered`, `TestAptHoldDecisions::test_declined_unhold_is_recorded_on_target_and_never_re_offered`, `TestAptHeldPackageSuppression::test_unrelated_recorded_decision_leaves_the_hold_set_intact`; `test_package_sync`:`TestBlockStateDecisionRoundTrip::test_skip_always_on_an_apt_hold_is_inert_next_run` |
| N5 | A snap refresh hold decided on its own marked machine-specific in run 1 | run 2 raises no hold item; the snap's own presence item stays live | U V | `test_block_state_decisions`:`TestSnapHoldDecisions::test_declined_hold_is_recorded_on_source_and_never_re_offered`, `TestSnapHoldDecisions::test_recorded_hold_does_not_silence_the_snaps_own_presence_diff`; `test_package_sync`:`TestBlockStateDecisionRoundTrip::test_skip_always_on_a_snap_hold_is_inert_next_run` |
| N6 | A flatpak mask decided on its own marked machine-specific in run 1 | run 2 raises no mask item | U | `test_block_state_decisions`:`TestFlatpakMaskDecisions::test_declined_mask_is_recorded_on_source_and_never_re_offered`, `TestFlatpakMaskDecisions::test_declined_unmask_is_recorded_on_target_and_never_re_offered`; `test_flatpak_sync`:`TestMaskSkipAlways::test_recorded_mask_produces_no_diff_on_the_next_run` |
| N7 | Any item declined for this run only in run 1 | run 2 offers it again, unchanged, and run 1 recorded nothing | U | `test_package_sync:TestCrossDirectionRoundTrips::test_install_propagates_then_reversed_removal_needs_approval` |
...
| N12 | Nomad has repository `S` and package `R` from it; Atlas has neither. Run 1 from Atlas, `R`'s removal declined | `S` is not raised at all — something on Nomad still uses it | ‼ | as C172: `test_apt_job`:`TestRepoRemovalWithheldWhileInUse::test_a_repository_whose_packages_removal_was_declined_is_not_deleted` proves `S` survives run 1; it is still raised in it |
...
| N17 | A snippet authored on Atlas in run 1; run 2 launched from Nomad, whose registry holds an entry Atlas lacks | the transfer stops and asks, naming the entries at risk; declining ends the run and sends nothing | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_lost_target_entry_prompts_and_proceeds_on_confirm` |
...
| N22 | Two machines, all four package jobs enabled, a full run | every job reviews before its own first change, one job's failure leaves the others' work intact, and the exit code reflects what failed | V | `test_package_sync`:`TestPackageSyncWholeRunContracts::test_one_failing_job_leaves_the_other_jobs_work_intact` (all four enabled, the job ordered first fails, apt's and snap's approved work still lands on Nomad, the run exits non-zero); `::test_each_manager_reviews_before_its_own_mutation` for review-before-own-change |
| N23 | A question covering a package and its hold answered permanently in run 1 | one answer leaves a mark on each holding machine, and run 2 raises neither the package nor the hold — the block does not come back on its own | U | `test_block_state_decisions`:`TestAMarkGivenOnAMergedQuestion::test_one_permanent_answer_writes_a_mark_on_each_holding_machine` |

## Where the tool does not do what the requirements say

Every row marked `‼`, except the four costs the criteria already accept — the two collateral limits and the two flatpak ones, recorded in [Where the tool does not yet meet these requirements](../planning/package-sync-conformance-criteria.md#where-the-tool-does-not-yet-meet-these-requirements) and not repeated here. Everything below is new against that section.

Each entry was found by one reading of the code and then put to an independent reader asked to refute it; what survived is what appears here, in the narrower wording that pass produced. None was observed on a running machine unless the entry says so, so each states what the code does, not what a run was seen to do.

**C172, N12 — a repository whose package's removal was declined is still raised.** `PKG-FR-REPO-DELETE` says that while anything on the target still uses a repository — counted after this run's **approved** removals — it "MUST NOT be raised as an item at all". Approved removals only exist once the review has returned, and the repository's own deletion is a question in that same review, so nothing can decide whether to raise it on a fact one of its own answers produces. The offer is therefore made on the plan-time reading (its only users are this run's removal CANDIDATES, C50), and `AptSyncJob._withhold_repositories_still_in_use` takes the approval back when a user of the file survives the review — so the file is never deleted with a package still installed from it, which is the half of the article that protects the machine. What remains is that the user was offered something that then did not happen. Closing it needs the question moved out of the batched review — either riding the package's own removal question, as a block rides its software (`PKG-FR-BLOCKS-REPLICATE`), or asked in a second round after the removals are decided, as late collateral already is. Both are design changes no article states.

**B27 — a hold outlives its package's mark and freezes a package that is not there.** `filter_inert` drops a marked package from Atlas's manifest, but the hold sets are read raw and `_drop_inert_diffs` looks up the hold's own id (`apt:hold:<name>`) against a mark recorded under `apt:package:<name>`. The hold item survives; `PackageConverger._hold_refusal` finds no package diff, reads that as "Nomad already has it", and runs `apt-mark hold` for a package Nomad does not have. The merged question does not reach this: a mark given there records both ids (`PKG-FR-BLOCKS-REPLICATE`), so what is left is a mark recorded against the package alone — the package marked in an earlier run, the hold arriving on Atlas afterwards. That is the bookkeeping hold `PKG-FR-APT-HELD-TARGET` calls harmful, and it blocks every later attempt to install the package.
...
**D39, D40, D41, D71 — apply-time collateral is refused where it should be asked, in two of three places.** `LateCollateral.ensure_asked` is real and wired into `AptSyncJob._build_work`, and it does put the three-way question before the first install — but only for an approved install that plan time could not rehearse because this run writes its repository. Collateral the real transaction reveals in `PackageConverger._install`, and the whole removal direction — `remove` has no late path at all — reach a bare `ConvergeItemFailed` instead. `PKG-FR-COLLATERAL-MANUAL` requires consent to the consequence, and `PKG-FR-ASK-AGAIN` permits the second question; nothing is lost either way, but the user is told rather than asked. (`OriginClassifier.target_resolvable`'s docstring still describes the behaviour before `LateCollateral` existed.)
...
- **C — the conflict question is gated on the proposed installs.** `PKG-FR-REPO-CONFLICT` says the question is raised only for a repository this run writes because an approved package comes from it, but the question must be asked before the approvals exist. The code uses the plan-time superset, so a file can be asked about and then not written. Nothing is written wrongly; no article covers the outcome.  
  A: Why "the question must be asked before the approvals exist"? PKG-FR-REPO-CONFLICT doesn't say that. Inventing a constraint that doesn't exist?
- **D47 — a mark recorded earlier in the same run.** `PKG-FR-COLLATERAL-MARKED` says such a mark must count and that the question must say the package is marked. The protection counts, but items are built before answers exist, so the question cannot name a same-run mark. Whether the second half is required is not decidable from the text.  
  A: "items are built before answers exist": this is an implementation choice, apparently not in line with requirements. Building the "items" and the questions is apparently not dynamic enough.
...
- **E49** — sideloads through a real run. Build a trivial `.snap`, or `snap try` a directory, on one machine, sync, and assert both machines' `snap list` are unchanged; worth it only if the fixture cost is acceptable, as the unit coverage of the branch set is already dense. VM.
...
- **E113, E115** — the `~/snap` boundary end to end. Put a stale revision directory under `~/snap/<app>/` on Atlas, sync with both jobs enabled, and assert Nomad has the active revision's directory and `common` but not the stale one; decline one snap's install in the same run and assert none of its revision directories arrive. VM.
...
- **J107** — a self-directed change from a manager other than apt. Nothing to write until such a case exists (ADR-021); the row is here so that whoever adds one has to log it the way J104–J106 do.
...
- **K67** — the second half of the row: that no job degrades to a reduced capture instead of refusing. The probe half is now audited statically; what a degradation looks like is per-job (apt's source-side `sudo find` returning empty digest maps is the one the code names), so this needs a per-job assertion that the degraded path is unreachable, not another audit. Unit.
...
- **K82** — the unit half is asserted; nothing shows it on two machines. Diverge a snap revision, answer skip, and assert `~/snap/<app>/<Atlas's revision>` never appears on Nomad while `common` still travels. VM.
- **K88** — the exclusion is asserted on the built command; nothing shows the file staying put on two machines. A non-interactive run reporting `manual_installs_sync` skipped, with `~/.config` inside a synced folder, must leave Nomad's registry byte-identical. VM.
- **K89** — the transfer half is now audited statically; what is left is the row's own wording, which says apt writes nothing inside the synced folders while its staging copy lives under `~/.cache/pc-switcher/apt-staging`. Assert the staging copy is removed on both the success and the failure path, and that `home.filter`'s `- .cache/*` covers the directory it leaves behind. Unit.
...
- The `folder_sync` boundary: no integration run enables a package job and `folder_sync` together, so neither the `~/snap` revision boundary nor the snippet registry's exclusion from the mirror is observed on real machines.
