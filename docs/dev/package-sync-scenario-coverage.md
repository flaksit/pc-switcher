...
| H126 | An item Nomad marked machine-specific is planned again — the machine holding the mark is the one being changed | It is proposed in no review, and it is neither removed from Nomad nor overwritten there — the mark protects it from every action the run takes of its own accord | P | `package_state:TestPipelineWiring::test_target_held_inert_item_absent_even_though_source_also_differs`; integration:`TestPackageSyncWholeRunContracts::test_skip_always_is_inert_in_both_roles` (reversed direction). The subject in both is an apt package, so the removal half is asserted and the overwrite half is not — that needs a marked `apt.conf.d` file the other machine differs on |
