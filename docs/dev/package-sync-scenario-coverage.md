...
| D56 | Same, but the removal's transaction only drifts onto `victim` after plan time, so nothing cancelled it | The go-ahead given for the install's consequence does not cover the removal's: that one raises its own question, and an unanswered one withdraws the removal while the install still runs — the consent is matched on the consequence, not the package | U | apt/collateral:`TestOnePackageTwoConsequences::test_the_apply_time_guard_matches_the_consequence_not_the_package` |
...
| E117 | E11's change is answered permanently ("this revision is this machine's own") | The mark is recorded on Nomad, whose revision the answer keeps | U | `test_block_state_decisions:TestSnapChangeDecisions::test_a_marked_revision_change_leaves_no_item_in_either_direction` |
| E118 | A later run finds the same two revisions, launched from either machine | `beta` produces no item of any kind — neither the change, nor the install or removal that one machine's copy surviving the mark would make of it | U | same test |
...
| H5 | `manual_installs_sync` plans (scan + registry read). | No mutating command and no `send_file` reaches Nomad while planning. | U | `manual_installs_sync:TestPlanIsReadOnly::test_plan_issues_no_mutating_command_and_transfers_nothing` |
...
| H167 | The map names a permanent decision for an unreproducible item. | The machine-specific mark is written and no snippet is: authoring one takes an editor, which the map has no way to stand in for. | U | `package_review:TestAutomationEnvCannotResolveAnUnreproducibleItem::test_a_permanent_answer_from_the_map_marks_the_item_and_writes_no_snippet` |
...
| J145 | A whole sync | What software Atlas has, and where it gets it from, is unchanged | U V | `test_package_sync_core:TestTheSourceIsNeverConverged::test_a_run_that_converges_the_target_issues_no_write_on_the_source`; `test_package_sync:TestASyncLeavesTheSourcesOwnSoftwareAlone::test_a_converging_run_leaves_the_sources_own_package_state_identical` |
...
| N24 | An item both machines have and disagree about — an `apt.conf.d` file, a snap's revision — is marked in run 1; run 2 is launched from the other machine | it is raised in neither run, in neither role. This is the one case where the direction of a run cannot name the holding machine, so the mark is read from both machines' records | U | `test_block_state_decisions`:`TestAptConfigOverwriteDecision::test_a_marked_overwrite_is_recorded_on_the_target_and_inert_in_both_directions`, `TestSnapChangeDecisions::test_a_marked_revision_change_leaves_no_item_in_either_direction` |

## Where the tool does not do what the requirements say

Empty. Every row still marked `‼` is one of the four costs the criteria already accept — the two collateral limits and the two flatpak ones, recorded in [Where the tool does not yet meet these requirements](../planning/package-sync-conformance-criteria.md#where-the-tool-does-not-yet-meet-these-requirements).
