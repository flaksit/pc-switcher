...
| A36 | Same branch as C24. | As C24. | P | `test_apt_origins:TestOriginClassification::test_unreplicable_origin_is_report_only_naming_the_origin` asserts no install, not the empty derived set |
...
| C3 | repository file byte-identical on both machines | no item and no write for the file itself. Whether a key is refreshed and whether the refresh runs turn on the key's own bytes, not on the file matching — an identical file over a rotated key still refreshes the key (C75) and still runs the unit (C156) | P | `test_apt_keyrings:TestKeysAreNotItems::test_a_matching_keyring_is_never_written` asserts no key write, no transfer and no `apt-get update` for the case where the KEY matches too; nothing asserts the absence of a write for the file alone |
...
| C171 | `vendor.list` differs, feeds a package Nomad marked machine-specific, and two approved installs — `pkg-a` and `pkg-b` — both take their origin from it; the user declines the overwrite | BOTH fail, each named, each saying the file was not written; neither is installed from Nomad's own version of the repository | — | `test_apt_job:TestRepositoryConflicts::test_skipping_a_conflict_writes_nothing_and_fails_the_package_that_needed_it` has one approved package, so "every approved package whose origin depended on it" is asserted for a set of one |
...
| E114 | `alpha` is on Atlas only at revision 10 and Nomad's snapd cannot fetch that revision; a second approved install is fine | `alpha` fails alone, naming it; the second snap still installs and the run continues | — | none. E50 covers the same failure on a `snap refresh` (a revision change); no test makes `snap install --revision=…` exit non-zero |
...
| F145 | Atlas holds `com.slack.Slack` at a version Nomad's copy of the remote does not offer, and the install is approved | The install names the reference and nothing else — no version, no commit — so Nomad ends on whatever build its own remote serves, and the difference is reported rather than forced | — | none. The command `FlatpakSyncJob.converge` builds is `flatpak install --assumeyes <scope> <remote> <ref>`; no test asserts the absence of a version or commit argument |
...
| F147 | Nomad masks `org.example.Blocked` in the system scope, Atlas masks nothing, and the user approves the unmask | The pattern is gone from Nomad's mask list afterwards — `flatpak --system mask` no longer prints it | P | `test_flatpak_sync:TestMaskConverge::test_system_scope_mask_removal_uses_sudo_and_remove_flag` asserts the `mask --remove <pattern>` command the approval issues; nothing reads Nomad's mask list back afterwards, on a VM or otherwise |
...
| G46 | A run with no terminal, and findings to resolve | Nothing is asked, no snippet is written, no mark recorded, and every finding ends the run skipped for this run — the third of the three states, since skipping for this run counts as a resolution and there is no fourth "unanswered" one | P | `test_package_review:TestUnreproducibleGroupResolution::test_non_interactive_offers_no_capture_and_marks_every_item_unresolved` asserts the `SKIP_ONCE` decision and, beside it, an internal `ReviewOutcome.unresolved` field. That field is a fourth state only in name: `manual_installs_sync` does not override `_unresolved_as_failures`, nothing reads it, and `execute` raises `JobSkipped` before `apply()` on such a run. No test asserts the run's own outcome, which is what the article is about |
...
| G93 | Nomad holds two entries Atlas does not have and a third whose body differs from Atlas's | One question naming all three — every entry the transfer would lose and every entry it would change | — | none. `_guard_registry_overwrite` builds `lost` and `changed` and `_render_overwrite_diff` renders both, but G70 seeds one lost entry and G71 one changed one; no test puts several in one question |
| G94 | Atlas has never had a snippet written on it and Nomad's registry holds two entries | No transfer happens, so no entry of Nomad's is lost or changed and no question is asked; Nomad keeps both entries | P | `test_manual_installs_sync:TestSnippetPush::test_absent_source_registry_makes_push_a_noop` asserts the absent source file sends nothing, but with no registry on Nomad — nothing asserts Nomad's own entries survive |
...
| N11 | Run 1 writes a repository for an install a late collateral answer then withdrew | the repository stays. The run names it by URL and filename and says nothing on Nomad installs from it, as neither a failure nor a warning. Run 2 does not offer it for deletion — Atlas still has the file | P | `test_apt_collateral`:`TestARepositoryWrittenForADeclinedInstall::test_the_repository_is_named_by_url_and_filename`, `::test_it_does_not_read_as_something_broken`, `::test_a_repository_a_surviving_install_still_needs_is_not_named` |
...
| N16 | A dependency-only package `Q` that Nomad's apt keeps or drops across an install-then-remove round trip of `P` | pc-switcher never proposes, installs or removes `Q` in either run | P | `test_apt_probe`:`TestManifestIsShowmanualOnly::test_auto_installed_dependency_produces_no_diff_of_any_kind` |
