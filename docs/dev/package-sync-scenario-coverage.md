...
| G46 | A run with no terminal, and findings to resolve | Nothing is asked, no snippet is written, no mark recorded, and every finding ends the run skipped for this run — the third of the three states, since skipping for this run counts as a resolution and there is no fourth "unanswered" one | ‼ | `test_package_review:TestUnreproducibleGroupResolution::test_non_interactive_offers_no_capture_and_marks_every_item_unresolved` asserts what the code does instead: `review_items`'s non-interactive arm decides every item `SKIP_ONCE` and ALSO returns each id in `ReviewOutcome.unresolved` |
