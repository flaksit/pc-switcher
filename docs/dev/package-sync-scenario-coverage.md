...
| # | Scenario | Expected | Cov | Test |
| --- | --- | --- | --- | --- |
| B16 | Nomad records a hold for `ghost` and does not have `ghost` | The run ends, naming `ghost` and Nomad and the `apt-mark unhold` that clears it; nothing on either machine is written first | U | `test_apt_job:TestAHoldWithoutItsPackageEndsTheRun::test_a_hold_the_target_records_without_the_package_ends_the_run` |
...
| D37 | The user SKIPPED `pkg-b`'s removal for this run; the approved removal of `pkg-a` would carry `pkg-b` off anyway | The user is ASKED, in the review's second pass — a candidate that was offered for removal and not approved keeps its protection. Nothing is asked in the first pass, where the batch is every candidate's own transaction; the question's reason names the ground, and keeping `pkg-b` leaves `pkg-a`'s removal unapplied rather than failing it | U | apt/collateral:`TestASkippedRemovalKeepsItsProtection::test_a_skipped_candidate_is_asked_about_rather_than_told_about` |
| D72 | Same question, answered "go ahead" | `pkg-a` is removed and the apply-time guard lets `pkg-b` go with it; `pkg-b`'s own removal item stays skipped, since that is not what was approved | U | apt/collateral:`TestASkippedRemovalKeepsItsProtection::test_going_ahead_at_that_question_runs_the_approved_removal` |
...
- Apt hold replication in every form: a hold that travels without a review line, a held package installed at Atlas's exact version, and the run a bookkeeping hold ends. The design rests on measured `apt-mark`/`apt-get` behaviour, and mocks assert only that the commands we think we send are sent.
