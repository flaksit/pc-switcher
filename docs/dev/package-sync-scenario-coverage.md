...
**Sections A–K default to one sync launched from Atlas** — that is how a row reads unless it says otherwise, and most do not. Two kinds say otherwise, and both say so in the row itself rather than leaving it to be inferred: about a dozen turn on what a *later* run does (e.g. B45, B47, C33, C61, C115, C119, D21, G35, G37, G82, G90, H114, H145 — "asked again next sync", "inert next run"), and four run with the roles swapped so Atlas is the machine being changed (A54, C175, H31, H126). A row that names no direction is the default.
...
| A15 | `code` is in Nomad's `apt-mark showmanual` from a hand `.deb`; Atlas does not have it | No apt item of any kind: no removal line, no review line and no `apt-get remove`. `apt_sync` owns nothing installed from a hand `.deb` "in any configuration", whichever machine holds it, and the general removal rule does not reach it | ‼ | none. `AptProbe.query_target_items` applies no hand-`.deb` exclusion — only `capture_source_items` does — so the name becomes an ordinary removal item |
...
| A40 | The origin URL carries a credential (`https://user:pw@repo.example.com/apt`) | The userinfo is withheld wherever the user reads the origin | ‼ | none. `messages.display_origin` strips the scheme and `redaction.redact_credentials` is anchored on `://`, so the two compose into a no-op and the credential survives into every origin line |
...
| A54 | `P` is on both machines; the user removes it on Nomad by hand, then syncs Nomad→Atlas twice — leaving the removal undecided, then approving it | Undecided leaves `P` installed on Atlas; approved removes it from Atlas. The roles are swapped for these two runs, so Atlas is the machine being changed | V | `test_package_sync:TestCrossDirectionRoundTrips::test_install_propagates_then_reversed_removal_needs_approval` |
...
| B7 | Atlas holds `pkg-a`, Nomad lacks it; the user approves the install and declines the hold | `pkg-a` is installed on Nomad at Atlas's version and is left unheld — the two decisions are independent in that direction too | U | `test_apt_packages:TestAHeldPackageIsInstalledAtTheSourcesVersion::test_an_approved_install_whose_hold_was_declined_leaves_the_package_unheld` |
...
| B35 | Atlas holds `pkg-a` and gets it from a repository this run cannot reproduce, so `pkg-a` is reported rather than installed; the user approves the hold | The hold fails alone, saying `pkg-a` is not on Nomad and the run cannot reproduce the repository it comes from | U | `test_apt_packages:TestAHoldNeedsItsPackage::test_a_hold_on_a_package_no_repository_can_supply_fails_alone` |
...
| E41 | Both machines have sideloads (different names, one each) | Each machine's sideloads are named — two findings, one per machine | U | `test_snap_sync:TestSideloadedSnaps::test_each_machines_sideloads_are_named_in_a_finding_of_their_own` |
...
| F118 | Nomad restricts a remote both machines have that no approved application needs this run | Nomad's filter comes off: the branch is stated without qualification, so a remote Atlas does not restrict does not stay restricted on Nomad whether or not this run installs from it | ‼ | none. `FlatpakSyncJob._clear_target_filters` and `_apply_remote_filters` run over the DERIVED remotes only, so a remote no approved application needs keeps whatever filter Nomad had. Unresolved: read this way the article contradicts `PKG-FR-FLATPAK-REMOTE-DERIVED`, which forbids syncing a remote no approved application needs — the criteria have to settle which governs before a test can be written |
...
| G5 | Atlas has a hand-installed `.deb` that apt marks automatically-installed (absent from `apt-mark showmanual`) | Presented as an item Nomad cannot get from any package manager: what the job covers is every installed version no configured repository supplies, and `apt-mark showmanual` is not the boundary the article draws | ‼ | none. `ManualInstallsSyncJob._scan_no_candidate_apt_packages` is handed `apt-mark showmanual`'s output and asks apt about nothing else, so an automatically-marked hand-`.deb` is invisible to both this job and `apt_sync` (A9) |
...
| G79 | Nomad's registry file is corrupt | Nomad is not read as holding nothing: the transfer does not proceed as purely additive, and nothing of Nomad's is overwritten without the user being asked | ‼ | none. `SnippetRegistry.load` degrades a corrupt file to an empty mapping (G64), so `_guard_registry_overwrite` finds nothing lost and nothing changed and pushes silently over it |
| G80 | Atlas's registry file on disk is corrupt at transfer time | Every Nomad entry counts as lost, so the user is asked — and approving sends the corrupt file | U | `test_manual_installs_sync:TestSnippetRegistryOverwriteGuard::test_a_corrupt_source_registry_makes_every_target_entry_a_loss` |
...
| H31 | A removal item is left declined for a whole run launched from Nomad, so the roles are swapped and Atlas is the machine being changed | Atlas keeps the package. The same item approved on the next run removes it, which is what makes the decline a decision rather than a deferral | V | integration:`TestCrossDirectionRoundTrips::test_install_propagates_then_reversed_removal_needs_approval` |
...
| H126 | An item Atlas marked machine-specific, on a later run launched from Nomad — so the machine holding the mark is now the one being changed | The item is still inert on Atlas: neither installed nor removed there, even if a decision is forced onto it | U V | `package_state:TestPipelineWiring::test_target_held_inert_item_absent_even_though_source_also_differs`; integration:`TestPackageSyncWholeRunContracts::test_skip_always_is_inert_in_both_roles` (reversed direction) |
...
**A14, A15 — apt offers to remove software it does not own.** `AptProbe.capture_source_items` drops packages installed from no configured repository and says the exclusion "happens HERE and nowhere else"; `AptProbe.query_target_items` applies no equivalent filter. A package installed from a downloaded `.deb` is therefore missing from Atlas's manifest and present in Nomad's, and diffs as a removal — whether both machines have it (A14) or only Nomad does (A15). `PKG-FR-DEB-OWNERSHIP` says `apt_sync` produces no item for such software in any configuration, which is the more specific rule and settles A15 against `PKG-FR-APT-REMOVE`'s general one; `PKG-FR-APT-SAME` independently rules out A14. The fix is one filter, and it costs a batched `apt-cache policy` over Nomad's manual set, which no run currently issues.
...
**A40 — the credential in an apt origin is never withheld.** `PKG-FR-CREDENTIAL-PRIVACY` requires the userinfo to be withheld wherever the system puts a URL in front of the user, and `ItemDiff` runs `redact_credentials` over every label and detail to do it. But `messages.display_origin` strips the scheme before the string becomes a detail, and `redaction.redact_credentials` returns early unless the text contains `://` — so for every origin line the redaction is a no-op. Measured here:

```
raw redacted   : https://***@repo.example.test/apt
display+redact : bearer:s3cr3ttoken@repo.example.test/apt
```

Every builder in `messages.py` that calls `display_origin` is affected — `build_origin_detail`, `build_repo_unavailable_detail`, `build_origin_mismatch_detail`, `build_origin_refusal_detail`. The builders that use the raw URI, `build_repo_removal_detail` and `build_stranded_repository_line`, still redact correctly. A private repository carries its password in its own address, so this is the case the article was written for.
...
**G5 — a hand-`.deb` that apt marks automatic is never detected.** `PKG-FR-MANUAL-SCOPE` covers every installed version no configured repository supplies. `capture_source_items` feeds only the `apt-mark showmanual` names into the no-candidate scan, so such a package reaches no review and no snippet can be written for it. Whether `apt_sync` then offers to remove Nomad's copy depends on Nomad's own manual mark — the A14 case.
...
- **B41 — a hold added and its package removed in the same run.** Both can be approved, because Atlas can hold a package outside its own manual set. The code removes and then holds, leaving Nomad holding what it no longer has. The articles say the hold is decided separately from its software; they do not say what happens when the two answers contradict.  
  A: Most intuitive if packages and holds are presented together: hold is mentioned in the same question as the package. The decision on the package is applied to the hold as well.
...
- **E — whether a warning satisfies "a run MUST name".** `PKG-FR-SNAP-SIDELOAD` requires a run to name the sideloads it found. They are named in a job warning, not in the review or the report. On the warning reading the branch is covered; on the report reading it is unmet.
  A: Sideloaded snaps do not need to be searched/found nor named. Update the user req doc, the conformance-criteria, code and any other doc.
- **F118 — the filter article's unscoped clause.** "No filter may be in force on a remote **this run provisions**" is scoped; "a remote the source does not restrict MUST NOT stay restricted on the target" is not. Read literally the second would require touching remotes no approved application needs, which `PKG-FR-FLATPAK-REMOTE-DERIVED` forbids. The code takes the narrow reading.  
  A: Make PKG-FR-FLATPAK-FILTER more precise: "a remote the source does not restrict MUST NOT stay restricted on the target" is only meant to be applicable for remotes that are needed by approved applications (or their dependencies).
- **F121 — a third-scope application and the remote count.** `PKG-FR-FLATPAK-THIRD-SCOPE` skips such an installation, but an application in it can still be the last user of a remote, and the parser cannot see it. Whether deleting that remote is acceptable is a decision, not a test.  
  A: Each installation (system-wide or a given user's per-user installation) keeps its own independent set of remotes, no? So you are raising a problem that doesn't exist: third-scope app will never refer to a remote that we touch/need and we will never touch a remote that a third-scope app uses, because the third-scope app uses a third-scope remote.
- **G79 — a registry on Nomad that cannot be parsed.** It reads as empty, so a transfer that would lose every entry counts as additive and asks nothing. `PKG-FR-READ-FAILS-JOB` says silence must not be read as an empty set; whether an unparsable file is silence or data is not stated.
...
- **G79** — Nomad's registry file unparsable while Atlas holds one entry. Assert whether a question is asked, and record the answer as the decision it is. Unit.
- **G80** — Atlas's on-disk registry unparsable while Nomad holds entries. Assert the question is asked and, if approved, that a corrupt file is what gets sent. Unit.
