...
sync_jobs:
  ...
  manual_flatpak_sync: false   # flatpak refs no remote can supply, via install snippets — see jobs/package-sync.md
  manual_installs_sync: false  # unowned installs under /usr/local and /opt, via install snippets — see jobs/package-sync.md
```

The package jobs (`apt_sync`, `snap_sync`, `flatpak_sync`, `manual_deb_sync`, `manual_snap_sync`, `manual_flatpak_sync`, `manual_installs_sync`) ship disabled: enabling any of them lets pc-switcher change installed software on the target, so it is opt-in. They have no per-job config sections — only the enable flags above. See [Package Sync](jobs/package-sync.md) for what they do.
