...
On a `--delete` mirror an exclude rule also protects a matching file that already exists on the target from being deleted or overwritten (rsync never deletes an excluded path — `--delete-excluded` is deliberately not used). The central `filter_file` is passed to rsync on every run, so its protection is immediate and unconditional. A per-directory `.pcswitcher-filter` is read by rsync from whichever side it is scanning, so on its own it can only protect the target once the file is present *on the target* — and a `--delete` mirror would otherwise delete (not protect) a target file a source-side rule names, turning even an update into a deletion. pc-switcher closes that gap: when a per-directory filter is not yet reflected on the target, it runs the mirror twice — first *without* `--delete` (which transfers the `.pcswitcher-filter` files, and everything else, onto the target while applying the full filter chain), then *with* `--delete`. Because the second pass now sees the per-directory files on the receiver, per-directory rules protect the target on the same sync, including the first sync and whenever you add or edit a `.pcswitcher-filter`. Both passes apply the identical central + per-directory filters, so nothing is transferred that either surface excludes. The extra pass is skipped in the steady state: before it runs, pc-switcher hashes just the `.pcswitcher-filter` files on each side and does the single deleting pass directly when they already match (so an ordinary sync — including the common case of no per-directory filters at all — stays single-pass). A `--dry-run` preview is taken without the seeding pass (dry-run writes nothing), so the preview can *over*-report deletions for per-directory-protected paths — the real sync deletes fewer files than the preview shows, never more. For rules that must never delete or expose something (secrets, machine-specific state), the central `filter_file` remains the authoritative place under GLOBAL-FIRST.
