...
- [package-sync.md](jobs/package-sync.md) - The seven package jobs (`apt_sync`, `snap_sync`, `flatpak_sync`, `manual_deb_sync`, `manual_snap_sync`, `manual_flatpak_sync`, `manual_installs_sync`): item -> diff -> review -> converge, per-manager review, machine-specific packages, install-or-update snippets and the installed-version snippet beside them
