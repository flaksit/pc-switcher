...
Any filter the **target** already had on that remote comes off first, before a single app installs — it would exclude the same apps, whoever set it. That is also how a remote you no longer filter on the source loses its filter on the target: the copy pass only knows the filters the source has, so a filter only the target carries would otherwise survive every run. The trade is that a run whose copy then fails leaves that remote unfiltered until the next one, and says so on every approved app from it.
