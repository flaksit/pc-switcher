...
If the source verifies a remote and has no key for it anywhere — neither its own keyring nor that directory — there is nothing to sync, and the run says so once. That remote refuses installs on the source too; the target ends up in the same state rather than a better-looking one.
...
Any filter the **target** already had on that remote comes off first, before a single app installs — it would exclude the same apps, whoever set it. That is also how a remote you no longer filter on the source loses its filter on the target: the copy pass only knows the filters the source has, so a filter only the target carries would otherwise survive every run. The trade is that a run whose copy then fails leaves that remote unfiltered until the next one, and says so on every approved app from it. Neither pass reaches a remote no approved app needs this run: a remote you install nothing from keeps the filter it has, on both machines.
