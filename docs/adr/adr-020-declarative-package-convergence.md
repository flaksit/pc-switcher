...
- A repository file, its signing keyring and a flatpak remote MUST be written as derived mechanism, never as review lines: they land because a package or ref approved in the review comes from them. Every `preferences.d` pin MUST be written whether or not a package derives it (D-36), and is equally never a review line when added or changed.
...
- `/etc/apt/apt.conf.d` MUST stay a reviewed item in all three directions (add, change, remove) with the full three-way decision and the machine-local registry. apt holds, snap refresh holds and flatpak masks MUST NOT be reviewed items in any direction and MUST NOT be recordable: they are derived, replicated because the software they apply to is (D-02). An apt hold naming a package its own machine does not have MUST end the run.
- A snap's revision or channel convergence MUST offer exactly two answers — converge, or skip once — and MUST NOT be recordable in a decision file.
...
- Only files apt itself reads may be captured, compared or written in any of its three fragment directories — `.sources`/`.list` under `sources.list.d`, no extension or `.pref` under `preferences.d`, no extension or `.conf` under `apt.conf.d`, and in all three only names apt's own character rule accepts. Anything else in those directories MUST be left alone.
...
- `manual_installs_sync` MUST detect by diffing: both machines are scanned and only what the target lacks becomes an item. Whether such an item counts as reproducible MUST be judged by whether the SOURCE holds a snippet, never by whether the target already does. The install-snippet registry (`~/.config/pc-switcher/package-snippets.yaml`) MUST be pushed to the target by `manual_installs_sync` itself and replayed there the same run — including a snippet authored on the fly during that run's review. A push that would lose or change an entry only the target holds MUST require explicit confirmation and abort the run when it cannot get it. A registry file on either machine that cannot be parsed MUST abort the run the same way, naming the file, and MUST NOT be read as holding no snippets; absent and empty still mean no snippets. `config_sync` MUST NOT carry the registry.
...
An item is something the user can meaningfully decide about, and it carries a stable identity through one diff → decide → apply pipeline. The item classes are the apt package, the apt config file, the snap, the snap channel, the flatpak ref, the unreproducible/manual install, and — for removal only — the apt source file and the apt pin file. A flatpak remote is not an item at all (D-41), and neither is a block — an apt hold, a snap refresh hold or a flatpak mask — though each keeps an identity of its own (below).

A block on a package (apt hold, snap hold, flatpak mask) keeps an identity of its own, so the converge order (install before hold, masks after refs) and the failure reporting can name it — but it is **derived**, not decided. It replicates because the software it applies to does, exactly as a pin does: a block changes nothing about what software exists, only about what may move, so replicating every one of them costs nothing and cannot get a per-item derivation wrong, while asking about one separately let the user give an answer that contradicted the one they gave about the software itself. Nothing about a block may be recorded machine-specific either; the mark that reaches it is the one given about its software. A block that freezes an installed copy — an apt hold, a snap refresh hold — is not registered for software the target does not end the run with; a flatpak mask is a rule about what may be installed and still lands.

An apt hold naming a package its own machine does not have is a bookkeeping failure rather than a state to replicate: it freezes nothing while refusing every later install of that name. The run ends, naming the package and the machine.
...
Report-only diffs (version mismatch, origin mismatch, an origin that cannot be provided) take no answer at all, as the first half of this decision says. They carry no converge verb, so there is nothing to apply and no holder machine for D-08a to record a permanent decision on, and a recorded answer would suppress the item entirely rather than stop reporting the drift the user meant. They are printed, and resolved by fixing the underlying condition.
...
An entry lasts as long as its item is on the machine holding it, and is dropped once that machine no longer has it (D-43) — including when this run's own approved work is what took it, which is the collateral case D-30 lets the user allow. Keeping a dead entry is not the cautious option it looks like: an entry acts in both roles, so one naming software the machine no longer has goes on refusing to install that software there, silently and for good, which is the opposite of what "keep my copy" was answered to. The check is a positive statement that the machine does not have the item — dpkg's installed set, the manager's own listing, the path itself — never absence from the narrower inventory the diff was built from, since apt's is `apt-mark showmanual` and a package that becomes automatically-installed leaves it while staying installed; a check that does not answer drops nothing. An item that comes back afterwards is a new item and is reviewed again, which is the price of the rule and the right one: the answer was about the copy that is gone.
...
Only files apt itself reads are captured, compared or written, in each of the three fragment directories by that directory's own rule (measured against apt 2.8.3, one filename at a time): `sources.list.d` is a directory users and packagers also drop `.save`, `.distUpgrade` and editor backups into, `preferences.d` and `apt.conf.d` get the same treatment from `dpkg`'s own `.dpkg-dist` files, and treating any of those as configuration would propose changes apt would never act on.
...
Batching is a strong preference, not a ban on asking twice. A job may ask again, including after it has begun writing, when the answer it needs rests on facts this run's own changes invalidated or that no read could establish before the first write. Correctness outranks the batching preference, and some things are knowable only once an action has landed. Asking twice BEFORE writing anything needs no such exception at all: rounds that come one after another with nothing done between them are what batching asks for.

`apt_sync` asks in rounds, and only `apt_sync`. Three of its questions are scoped by their own articles to the work this run APPROVES rather than to what it proposes — a repository is not raised for deletion while anything the run leaves installed still uses it, a repository conflict is raised only for a file an approved package makes travel, and a package the user marks as the target's own is protected from that moment. None is decidable while the plan is built and all three are decidable the instant the first round's answers exist, so they are a second round: same sitting, nothing written yet, and a stopping answer there still stops everything. Building every item before any answer existed is what made those three approximations of the articles they come from.

A package's classification depends on the SOURCE's origins, which no run mutates, so no fact an earlier round is answered on can be invalidated by what the run writes; the one fact that does depend on those writes — whether the target really ends up with the source's origin — is checked by D-35 and reported as a per-item failure rather than re-asked. What genuinely cannot be known before writing is the collateral of an install whose repository this run itself writes: until that write lands the target's apt has never heard the name (D-40). That is a third round — asked once, in one batch, before the first of those installs, and the only one that needs the permission to ask after a change. The other three jobs have one round and need no more.
...
Detection covers apt packages whose installed version comes from no configured repository — a bare `.deb`, whose only origin is dpkg's own status file — and unowned installs under `/usr/local` and `/opt`, and is owned by `manual_installs_sync`. It is a diff, not a scan of the source alone: both machines are scanned and only what the target lacks is presented. That is what stops a secondary path — a symlink in `bin` pointing at an application in `/opt` — being asked about again once the snippet has produced it on the target. An install snippet is an opaque text blob replayed non-interactively through the existing executor with the exit code deciding success — the tool never parses, versions, diffs or reasons about snippet content. Snippets live in the shared, synced config (`~/.config/pc-switcher/package-snippets.yaml`) and cover bare `.deb`s and manual installs only; snap and flatpak items do not carry snippets (YAGNI — every current one comes from a reachable remote).
...
Package jobs export their owned paths to `folder_sync` via the ADR-018 mechanism, which turns them into non-overridable filters without knowing anything about either ecosystem: `flatpak_sync` owns `~/.local/share/flatpak`, `snap_sync` owns the `~/snap/<app>/<rev>` revision dirs, and `packages.state` owns both the machine-local decision files and the install-snippet registry.

The rule for the snap dirs is the target's own state, not this run's intent: a revision dir is mirrored only where the target is itself active at that revision, which `folder_sync` establishes by reading the target's `snap list` once, after the package jobs have run (`PKG-FR-JOB-ORDER`). That mirrors the active revision's per-user app data, which is the whole point of converging the revision (D-06), and excludes everything else — the retained older dirs, an app whose revision change or install the user declined or that failed, and every dir on a run with `snap_sync` disabled. An indeterminate active revision, or a target whose snapd could not be asked, excludes all of that app's revision dirs as the safe default. The earlier design gated the exclusion on `snap_sync` being enabled and always mirrored the active revision, on the assumption that the job had converged it; a declined item, a failed one and a disabled job each break that assumption and plant data for a revision the target's snapd never installed, which `PKG-FR-SNAP-DATA-BOUNDARY` forbids. The cost of the correction is that two machines that happen to agree on a revision without `snap_sync` running no longer get that data mirrored — accepted, because nothing in that run establishes that they agree.

The snippet registry is excluded from the mirror outright, on every run. It does travel (`PKG-FR-REGISTRY-SYNCS`), but only as `manual_installs_sync`'s own push, which names the entries an overwrite would lose or change and asks first (`PKG-FR-REGISTRY-CONSENT`) and which a skipped or non-interactive run never makes. A mirror of the same file is that transfer with the question removed, so it is excluded whether or not `manual_installs_sync` is enabled.
...
The converse is not a failure and not a rollback. A repository this run already wrote for an install the user then declined — at the second collateral round (D-40) or anywhere else — stays where it is: removing it is a change nobody approved. Where no surviving approved install needs it, the run names the file and the URLs it declares and says nothing on the target installs from it, at INFO. Every item the user decided about ended as they decided, so there is nothing to report as wrong.
...
A consent is keyed to the consequence it was given for — the causing change, what that change does, and the package it happens to — never to the package alone. One protected package can be the casualty of the install batch and of the removal batch in one run, and agreeing to lose it once is not agreeing to lose it twice.
...
What covers it instead is a second question, asked after `/etc/apt` has converged and `apt-get update` has run — where apt CAN resolve the package — and before the first of those installs. It offers the same go-ahead / keep-the-package / stop-the-sync answers as the plan-time one, batched over all of them at once, which is what D-24's permission to ask again exists for: the facts that question needs do not exist while `plan()` runs. A package whose origin can never be replicated needs no rule: it is `REPO_UNAVAILABLE`/`REPORT_ONLY` and is never an install candidate.
...
- A package whose repository this run derives gets its collateral question after `/etc/apt` has converged rather than with the rest of the review (D-40), so `apt_sync` can interrupt the user once more after it has begun writing.
...
- A filtered flatpak remote replicates WITH its filter. flatpak records the filter's path rather than its content (measured), so the content is an ordinary file the run carries byte-for-byte exactly as it carries a keyring: written to the same absolute path on the target and applied there before that remote's refs install. The cost is that the job writes one path per filtered remote that it does not otherwise own, and that a source whose filter denies an application it has itself installed ends the run rather than being worked around.
