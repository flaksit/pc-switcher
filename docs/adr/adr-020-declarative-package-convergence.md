...
`apt_sync` asks twice, and only `apt_sync`. A package's classification depends on the SOURCE's origins, which no run mutates, so no fact the first round is answered on can be invalidated by what the run writes; the one fact that does depend on those writes — whether the target really ends up with the source's origin — is checked by D-35 and reported as a per-item failure rather than re-asked. What genuinely cannot be known first is the collateral of an install whose repository this run itself writes: until that write lands the target's apt has never heard the name (D-40). That is the second round — asked once, in one batch, before the first of those installs. The other three jobs have no second round and need none. Where the facts genuinely do change, a second question is the correct answer.
