...
Every screen states its answers as effects on a NAMED machine, and never as the tool's own vocabulary for the two ends of a run. The two machines are identified by hostname in every title, detail, prompt and answer the user reads; "source" and "target" stay in code, docstrings and logs. Concretely: "keep it on `fleksi`" rather than "skip once" where there is something on that machine to keep, "`p17` has 1.0-1, `fleksi` has 2.0-1" rather than "source has …, target has …", and the two panels of a conflict prompt titled with the machines that hold them. The user cannot answer a question about "the target" without first working out which of their computers that is, and every such translation is a chance to answer the wrong way.
