...
The source captures a manifest of package-related items; the target diffs its own state against it and converges using `apt`, `snap` and `flatpak` themselves — package databases are never rsynced. A package replicates as (name, origin), not name: the review decides packages, holds, masks and apt config — a hold or mask riding its software's own question wherever that software is itself an item in the run — while repositories, their keys and flatpak remotes are derived from the packages approved from them, every `preferences.d` pin syncs whether a package derives it or not, and none of them is reviewed when added or changed — a flatpak remote is never reviewed at all. Four independent jobs (`apt_sync`, `snap_sync`, `flatpak_sync`, `manual_installs_sync`) each run plan then review then apply inside their own `execute()`, with every change approved before it is made and the questions batched into one screen per manager per action.
...
- `/etc/apt/apt.conf.d`, apt holds, snap holds and flatpak masks MUST stay reviewed items in all three directions (add, change, remove) with the full three-way decision and the machine-local registry. A hold or mask whose software is itself an item in the same run MUST be presented in that software's own question and take that software's decision, on both machines' records (D-02).
...
That separation is about identity, not about how the question is put. Where the software a block applies to is itself an item in the same run, the two are presented as one question and the software's answer governs both — including the permanent one, which is recorded against both ids, each on its own holding machine (D-08a). Two questions there let the user give two answers that contradict each other with nothing showing that they do: the source can hold a package it does not itself have, so one run can carry an approved removal and an approved hold for the same name, and applying both leaves the target holding what it no longer has. Blocks keep their own ids throughout, so the converge order (install before hold, masks after refs) and the decision files are unchanged. A block that freezes an installed copy — an apt hold, a snap refresh hold — is not registered for software the target does not end the run with; a flatpak mask is a rule about what may be installed and still lands, and a mask, naming its software by pattern, rides a question only where the pattern covers exactly one of the run's items.
