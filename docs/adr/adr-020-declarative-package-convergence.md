...
The source captures a manifest of package-related items; the target diffs its own state against it and converges using `apt`, `snap` and `flatpak` themselves — package databases are never rsynced. A package replicates as (name, origin), not name: the review decides packages, holds, masks and apt config, while repositories, their keys and flatpak remotes are derived from the packages approved from them, every `preferences.d` pin syncs whether a package derives it or not, and none of them is reviewed when added or changed — a flatpak remote is never reviewed at all. Four independent jobs (`apt_sync`, `snap_sync`, `flatpak_sync`, `manual_installs_sync`) each run plan then review then apply inside their own `execute()`, with every change approved before it is made and the questions batched into one screen per manager per action.
