...
## Which files apt reads in its three fragment directories (`PKG-FR-APT-IGNORES`)
...
## A pin, not just a repository, decides which origin wins (`PKG-FR-PIN-ALWAYS`)
...
## An unattached target fails on ESM at install time, not refresh time (`PKG-FR-ESM-GATE`)
...
## apt holds block install, upgrade and removal alike (`PKG-FR-APT-HOLD-VERSION`, `PKG-FR-APT-HELD-TARGET`)
...
## snap resolves one name to one publisher (`PKG-FR-SNAP-IDENTITY`)
