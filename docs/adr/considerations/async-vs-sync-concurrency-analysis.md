...
1. **Modules run synchronously** in main thread - can't be interrupted mid-operation
...
- Sequential module execution with state machine
- Signal handling (SIGINT/SIGTERM) that calls `abort(timeout)` on current module
- Disk monitoring in background thread
- Module lifecycle: validate → pre_sync → sync → post_sync → abort (on error/interrupt)
...
- `KeyboardInterrupt` caught in `run()` method which calls `cleanup_current_module()`
- Modules run synchronously in main thread

**Module Interface (`/home/janfr/dev/pc-switcher/src/pcswitcher/core/module.py`):**
- `abort(timeout: float)` is a synchronous method
- No threading primitives in base interface
- Modules track `_aborted` flag internally

**Dummy Success Module (`/home/janfr/dev/pc-switcher/src/pcswitcher/modules/dummy_success.py`):**
...
1. **Abort timing guarantee mismatch**:
   ...
   - **Reality**: If module is in `time.sleep(1)`, it won't see the abort request for up to 1 second. If module calls `subprocess.run()` or `remote.run()` with 30-second timeout, it blocks until that completes.

2. **Force-kill mechanism absent**:
   - **Docs**: FR-003 says "orchestrator MUST force-kill the RemoteExecutor connections and module thread"
...
### Operations Modules Currently Perform
...
### Real-World Sync Operations (Future Modules)
...
- Module should see abort request within seconds (not 30s+)
...
5. Exception caught in `run()`, calls `cleanup_current_module()`
6. `cleanup_current_module()` calls `module.abort(timeout=5.0)`
7. Module's abort() sets flag but doesn't kill running processes
...
**Current Reality**: Could be unbounded. If module calls:
```python
...
1. Kill local subprocesses started by module
...
- No race conditions in module code
...
### Option B: Modules Run in Separate Threads

**How It Works**:
- Orchestrator spawns thread for each module execution
...
```python
# In Orchestrator
def _execute_module_with_timeout(self, module):
    with ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(module.sync)
        try:
...
- Must rewrite entire module interface
...
1. **Everything is async** - All module methods are coroutines
2. **Task-based cancellation** - Each module runs as an `asyncio.Task` that can be cancelled
...
graph TB
    subgraph EventLoop["Asyncio Event Loop"]
        O[Orchestrator Task] --> MT[Module Task]
    ...
    subgraph Cancellation["Cancellation Flow"]
        DM --> |"on low space"| Cancel[Cancel Module Task]
        SM --> |"on SIGINT"| Cancel
...
classDiagram
    ...
    class AsyncOrchestrator {
        -_session: SyncSession
        -_modules: list~AsyncSyncModule~
        -_current_task: asyncio.Task
        -_ssh_connection: asyncssh.SSHClientConnection
        +async run() void
        +async cancel_current_module() void
        -async _execute_module(module) void
    }
    ...
    AsyncOrchestrator --> AsyncSyncModule : manages
    AsyncSyncModule --> AsyncRemoteExecutor : uses
...
sequenceDiagram
    ...
    MT->>O: Module task completed (cancelled)
    O->>O: Log cancellation, continue cleanup
```

#### Module Implementation Pattern for Option C
...
class AsyncSyncModule(ABC):
    """Abstract base class for async sync modules."""
    ...
    async def validate(self) -> bool:
        """Validate module can run. Returns True if valid."""
        ...
...
class DummySuccessModule(AsyncSyncModule):
    """Example async module implementation."""
...
class AsyncOrchestrator:
    """Orchestrates async module execution with cancellation support."""

    def __init__(self, config: Config, session: SyncSession):
        self._config = config
        self._session = session
        self._modules: list[AsyncSyncModule] = []
    ...
    async def _execute_module(self, module: AsyncSyncModule) -> None:
        """Execute a single module as a cancellable task."""
        self._current_task = asyncio.current_task()

        try:
            if not await module.validate():
                raise SyncError(f"Module {module.name} validation failed")

            await module.pre_sync()
            await module.sync()
            await module.post_sync()

        except asyncio.CancelledError:
            self._logger.info(f"Module {module.name} was cancelled")
    ...
    def _handle_sigint(self) -> None:
        """Handle SIGINT by cancelling current module."""
    ...
    async def cancel_current_module(self) -> None:
        """Cancel the currently executing module."""
        ...
        while True:
            ...
            for path in self._paths:
                if not self._has_sufficient_space(path):
                    ...
                    await self._orchestrator.cancel_current_module()
                    return
...
flowchart TD
    ...
    subgraph TaskGroup["TaskGroup Context"]
        ...
        TaskGroup --> LoadMod[Load Async Modules]

        LoadMod --> ExecLoop{For each module}

        ExecLoop --> |Next module| Validate[await module.validate]
        Validate --> |Valid| PreSync[await module.pre_sync]
        PreSync --> Sync[await module.sync]
        Sync --> PostSync[await module.post_sync]
        PostSync --> ExecLoop
    ...
    subgraph CancellationPath["Cancellation Path"]
        ...
        CancelledError --> Cleanup[await module.cleanup]
...
|-----------|---------|---------------|
| `SyncModule` | Synchronous methods | All methods are `async def` |
...
3. Convert `SyncModule` interface to async
4. Convert `Orchestrator` to async with TaskGroup
5. Implement async disk monitor
6. Convert all existing modules to async
...
### Option D: Hybrid - Sync Modules with Subprocess Tracking/Killing

**How It Works**:
- Modules remain synchronous (simple programming model)
- Add subprocess tracking to module base class
...
def _handle_interrupt(self, signum, frame):
    ...
    if self._current_module:
        for proc in self._current_module._tracked_processes:
            proc.kill()
...
1. **Keep synchronous module execution** (simple mental model)
...
classDiagram
    ...
    SyncModule --> ProcessRegistry : uses
...
sequenceDiagram
    ...
    participant M as Module (in main thread)
    participant TC as TargetConnection

    Note over M: Module running sync()
...
#### Module Implementation Pattern for Option D
...
class InterruptHandler:
    ...
    def trigger_abort(self) -> None:
        ...
        # Set module abort flag
        if self._current_module:
            self._current_module._aborted = True
...
flowchart TD
    ...
    StartDM --> LoadMod[Load Modules<br/>Inject Registry]
    LoadMod --> Validate{Validate All Modules}

    Validate --> |Valid| Execute[Execute Modules]

    subgraph ModuleExec["Module Execution (Main Thread)"]
        Execute --> PreSync[module.pre_sync]
        PreSync --> Sync[module.sync]
        Sync --> |Uses| RunSub[run_subprocess]
        RunSub --> |Tracks| Registry[(Process Registry)]
        Sync --> PostSync[module.post_sync]
    end

    PostSync --> NextMod{More Modules?}
    NextMod --> |Yes| Execute
...
3. **Simple module code** - Authors use `run_subprocess()` helper, tracking is automatic
...
|-----------|---------|-------|
| `SyncModule` | No process tracking | `ProcessRegistry` injected, helper methods |
...
| `Orchestrator` | No process awareness | Owns `ProcessRegistry`, passes to all modules |
...
2. Enhance `SyncModule` base with tracking helpers
3. Update `InterruptHandler` to use registry
4. Connect disk monitor callback to abort
5. Update existing modules to use new helpers
...
**Requirement**: Call abort(timeout) on current module, log "Sync interrupted by user" at WARNING level, exit with code 130
...
- Modules don't report what they started on target
...
1. **Add subprocess tracking to SyncModule base class**
...
3. **Implement remote process tracking**
   - Modules report PIDs of processes started on target
...
2. **Convert module interface to async**
