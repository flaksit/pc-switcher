...
- Where the exit code is ambiguous because a legitimate state and a real failure share it, the COMMAND MUST be reshaped so the exit code becomes unambiguous, in preference to guessing from output or from stderr text.
...
One command sits on the line and stays on the per-item side of it: `apt-get --dry-run`. Measured in a stock `ubuntu:24.04`, a name apt cannot locate exits **100** with `E: Unable to locate package`, which is the same exit code a held dpkg lock produces, and D-03's remedy — reshape until the exit code is unambiguous — is unavailable, because apt offers no second code and no second mode. So the classification falls to whichever cause dominates each call site, and at both of `apt_sync`'s it is the request. Apply time simulates one approved install or removal, where apt's refusal is a fact about that request (ADR-021 D-27), and a lock met there cannot be undone by failing fast — the items already converged stay converged. Plan time simulates the whole candidate set, which routinely names a package whose repository this run has not written yet (an ADR-021 §2.3 class-3 install), so a non-zero exit there is the ORDINARY answer; promoting it to `ProbeFailed` would fail every run that carries a new vendor's package.
