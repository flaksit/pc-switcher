...
pc-switcher's own install-snippet registry is not a package manager's answer, and it is the one read whose failure is not capped at its job. A `package-snippets.yaml` that cannot be parsed raises `SyncAborted` from `packages.state`, naming the file and the machine (`PKG-FR-REGISTRY-CONSENT`). The plain class, not its `SyncAbortedByUser` subclass: nobody was asked anything, so nothing printed about this run may report the user as having stopped it (GitHub issue #224). Two things make it different from every read D-06 covers: nothing else in the run can repair it, but the USER can — it is a small file they wrote, on a machine they have — and the fix has to land before the next sync reads it. A `ProbeFailed` would fail `manual_installs_sync` and let the run carry on, which reads as a passing sync with one job down rather than as "go fix this file". An absent or empty registry stays ordinary data.
