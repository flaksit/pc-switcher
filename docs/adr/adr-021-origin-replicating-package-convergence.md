...
- **Withholding the two files silently, as D-38 first ruled** — rejected: the transactional-rollback premise behind it is refuted by measurement, and what remains is a choice between an apt without ESM and an attachment only the user can perform, which is the user's to make. Withholding was also rejected as the no-TTY fallback, for a second reason — the ESM pins travel regardless, so the target ends up with pins over a repository set neither machine has.
