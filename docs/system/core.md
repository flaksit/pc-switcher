...
"Not purely read-only" is deliberately wider than "changes a file". An operation is exempt only when it can change no state on the machine at all — no file content, no process state, no lock or other advisory state, no package-manager database, no credential cache. Scoping the prompt to content alone leaves out the operations that seize control of a machine without writing anything: a `flock` that takes the sync lock, a background process left running, a `sudo` probe that primes the credential cache.
