...
- **Responsibilities**: the manually-installed apt package set (`apt-mark showmanual`, not the full dpkg selection — apt resolves dependencies on the target), plus the repository state that governs where packages come from: sources (`/etc/apt/sources.list.d`), pins (`/etc/apt/preferences.d`) and apt config (`/etc/apt/apt.conf.d`). Signing keys (`/etc/apt/keyrings`, legacy `/etc/apt/trusted.gpg.d`) are managed too, but not as items — see the convergence verbs below. Also detects unreproducible items: apt packages with no repository candidate, and unowned installs under `/usr/local`/`/opt`.
...
- **Item classes**: `APT_PACKAGE`, `APT_SOURCE`, `APT_PIN`, `APT_CONFIG`, `APT_HOLD`, plus `UNREPRODUCIBLE` for items neither apt nor any package owns. Signing keys are deliberately not an item class (ADR-020's 2026-07-27 amendment): the user decides about repositories, and keys follow.
- **Convergence verbs**: `apt-get install`/`apt-get remove` for packages (never `purge`), each preceded by an `apt-get -s` transaction simulation that refuses the real command if it would touch an unapproved package; `sudo install` (staged under the target's own `~/.cache/pc-switcher/`, never a direct SFTP write into `/etc`) for repository/pin/config files, bracketed by two key operations nobody reviews — provisioning the keyrings the surviving repositories reference (byte-for-byte from the source, content-based so a rotated key travels) before any source write, and deleting the `/etc/apt/keyrings` files no surviving repository references after every source write and deletion, only when the run removed a repository — followed by a single `apt-get update` and a full rollback of the whole repository-file group if that update fails (each restore is a separate command whose result is checked, so a step that fails is reported by file name; when any step fails the backup directory is kept rather than discarded, since it then holds the only copy of that file's pre-run content, and the group's failure text reads `ROLLBACK INCOMPLETE` regardless of what the re-probe says); a snippet replay (see the data model doc) for a resolved unreproducible item.
...
- **`validate()` checks**: `snap version` on both source and target; passwordless sudo on both source and target (the target installs snaps, and both ends get the sync-window auto-refresh pause); a read-only `sudo snap get system refresh.hold` probe on both machines, logged as context only — never acted on. The probe runs under sudo because snapd admin-gates *reading* snap configuration; unprivileged it fails rather than reporting "no hold".
...
- **Convergence verbs**: `flatpak remote-add --if-not-exists`/`flatpak remote-delete` for remotes, always before the ref installs that depend on them; `flatpak install -y`/`flatpak uninstall -y` for refs, each prefixed with `sudo` if and only if the item's own scope is `system`. A ref whose origin remote is neither already on the target nor among this run's own successfully-added remotes is refused with a per-item failure naming the missing remote, rather than issuing an install flatpak would reject. A remote offered for deletion is never refused that way — its review item's detail names the target refs that still have it as their origin in the same scope, so the consequence is stated before approval.
...
Coverage is every modification pc-switcher makes to either machine except `folder_sync`'s rsync mirror pass (#209): each package manager's converge commands, apt's repository-file backup/stage/promote/rollback sequence, the machine-local decision files on both machines, the install-snippet registry and its push, the orchestrator's snapd auto-refresh pause and its restoration, btrfs snapshot creation and deletion, the config push to the target, the target's lock claim, the installer run on the target, and the VS Code state DB replacement.
