...
- **PKG-FR-DEB-OWNERSHIP**: Software no repository can install MUST belong to `manual_deb_sync` alone. `apt_sync` MUST NOT produce an item, a review line or an install for it in any configuration. Which package that is MUST be decided once per package name and applied to both machines' manifests: the source's answer where the source has the package, the target's own where only the target has it. One name MUST NOT get one verdict per machine.  
  Lineage: 002-package-sync, ADR-020-D-DEB-OWNERSHIP, GitHub #185, #285  
  Impl: `packages/apt_policy.packages_no_repository_can_install`; `AptSyncJob.plan` withholds the name from both manifests
...
- **PKG-FR-SOURCE-INTENT**: The source machine's state MUST be the only statement of intent, and the target MUST NOT decide anything. A sync MUST NOT change what software the source has, nor where it gets it from. The writes a sync does make on the source are exactly four, each required by an article of its own: a machine-specific mark (`PKG-FR-MACHINE-SPECIFIC`), a write to the snippet registry (`PKG-FR-MANUAL-SAME-RUN`'s authored snippet, `PKG-FR-DEB-AMBIGUITY`'s deleted entry), the snap refresh pause (`PKG-FR-SNAP-REFRESH-PAUSE`), and the apt update-timer pause (`PKG-FR-APT-TIMER-PAUSE`). All four are covered by `PKG-FR-CONFIRM-EACH`.  
  Lineage: 002-package-sync
...
- **PKG-FR-APPLY-FLAGS-SCOPE**: Neither option may answer a question the source's state cannot settle: `PKG-FR-REPO-CONFLICT`, `PKG-FR-APTCONF`, `PKG-FR-MANUAL-SCOPE` (`PKG-FR-SNIPPET-VERBATIM`), `PKG-FR-ESM-GATE`, `PKG-FR-REGISTRY-CONSENT`, `PKG-FR-DEB-AMBIGUITY`, and anything a package job asks that is not a review item. Each MUST be left where a run with nobody to ask leaves it — declined for this run and named — except the registry transfer, which MUST still abort the run. A report-only finding is unaffected.  
  Lineage: 002-package-sync
...
- **PKG-FR-VERSION-SNIPPET**: A registry entry MUST carry the install-or-update snippet replayed on the target, whatever its kind. It MUST carry an installed-version snippet, printing the version installed on whichever machine runs it, where and only where its kind has no other version source — an unowned path alone. An entry missing a body its kind requires, or carrying one its kind does not, MUST be treated exactly as an unparsable registry is — the run ends naming the file — and MUST NOT be completed or corrected by a default of any kind. Authoring MUST capture exactly the bodies that kind requires, MUST offer the existing content of each where there is one, and MUST show each captured body back in full once its editor closes. The installed-version snippet MUST run on BOTH machines while the run is planning, MUST be treated as read-only without that being verified, and MUST NOT be gated behind the per-command confirmation. Its output MUST be withheld on the same terms as any other, and MUST NOT be parsed or interpreted as anything but a string.  
  Lineage: 002-package-sync, ADR-020-D-UNREPRODUCIBLE-ITEMS  
  Impl: `items.carries_version_body` decides which bodies; `_capture_bodies`  
...
- **PKG-FR-MANUAL-SOURCE-DECIDES**: Whether a presented item is reproducible MUST be decided by the source's snippet registry alone. An item with a snippet only on the target MUST still be treated as unresolved. This binds resolution, not detection.  
  Lineage: 002-package-sync
- **PKG-FR-DEB-AMBIGUITY**: Where the snippet registry holds an entry for an apt package the SOURCE has installed and the source's apt has a repository it can install that name from, the system MUST ask which of the two put it there and MUST NOT decide for the user. The question MUST be raised on the source's verdict alone, MUST show the recorded snippet and what apt says it would install, and MUST offer exactly two answers: that apt installed it, which MUST delete that entry from the registry on both machines; and that the user installed it, which MUST keep the entry and MUST say how to make apt stop offering the package. Nothing about the question or its answer may be recorded — no marker, no decision-file entry, no registry flag — so the same state MUST raise it again on every run. An item recorded machine-specific MUST NOT be asked about, and neither `--apply-package-installs` nor `--apply-package-removals` may answer it. This question MUST NOT be extended to snap or flatpak.  
  Lineage: 002-package-sync, ADR-020-D-DEB-OWNERSHIP, GitHub #285  
  Impl: `ManualDebSyncJob.ownership_questions`; `SNIPPET_OWNERSHIP_REVIEW_ACTION`; `UnreproducibleSyncJob._drop_snippets_the_package_manager_owns`  
  Rationale: see [package-sync-rationale.md#pkg-fr-deb-ambiguity](../adr/considerations/package-sync-rationale.md#pkg-fr-deb-ambiguity)
...
| [Software no manager can reproduce](../planning/package-sync-user-requirements.md#software-no-manager-can-reproduce) | `PKG-FR-MANUAL-RESOLUTION` `PKG-FR-MANUAL-DIFF` `PKG-FR-MANUAL-OPT-SHAPE` `PKG-FR-MANUAL-SOURCE-DECIDES` `PKG-FR-DEB-AMBIGUITY` `PKG-FR-MANUAL-SAME-RUN` `PKG-FR-SNIPPET-VERBATIM` `PKG-FR-REGISTRY-SYNCS` `PKG-FR-REGISTRY-CONSENT` `PKG-FR-MANUAL-FAIL-ITEM` |
