...
- **PKG-FR-BLOCKS-REPLICATE**: An apt hold, a snap refresh hold and a flatpak mask MUST each replicate as an item decided separately from the software it applies to — EXCEPT where that software is itself an item in the same run, in which case the two MUST be presented as one question that names both, and the decision given there MUST govern both. Which case a block is in is decided by whether its software carries a decision this run: software that is only reported is no item, and a block that names its software by pattern rather than by name rides a question only where the pattern covers exactly one of this run's items. The merged question MUST take the software's own direction and therefore its default (`PKG-FR-HARMLESS-DEFAULT`). Applying it MUST converge the software as its own article requires and the block to the source's state — except that a block which freezes an installed copy, an apt hold or a snap refresh hold, MUST NOT be registered for software the target does not end the run with, and MUST there be reported as not applied rather than as a failure. Declining for this run MUST leave both unchanged and record nothing. A machine-specific mark given on the merged question MUST be recorded for the software AND for the block, each on its own holding machine (`PKG-FR-MACHINE-SPECIFIC`).
  Why: the two answers can contradict each other, and while they are two questions nothing shows the user that they do. The source can hold a package it does not itself have, so one run can carry an approved removal and an approved hold for the same name, and applying both leaves the target holding what it no longer has.
  Why the mark covers both: recording only the software would leave the block to be asked on its own on the next run — about software the tool has just been told to leave alone.
  Why the software's default governs: a block is not software, so a block that comes off as part of an approved install engages neither `PKG-FR-REMOVAL-DISTINCT`, which is about deleting software, nor the overwrite half of `PKG-FR-HARMLESS-DEFAULT`, which is about replacing content the user wrote. In the other direction the merge only ever makes the default safer, since a block that rides a removal inherits the removal's skip.
  Why the exception is narrow: a block whose software is no item this run is the ordinary case — both machines already have the software — and there it is a standing preference the user holds on its own, with nothing to attach it to.
...
- **PKG-FR-APT-HOLD-ITEM**: An apt hold MUST be an item separate from the package it applies to, decided separately, both when it is added and when it is removed — EXCEPT where the package is itself an item this run, whether an install, a removal or a change, in which case the two MUST be one question and the package's decision MUST govern both (`PKG-FR-BLOCKS-REPLICATE`). A hold MUST NOT be registered for a package the target does not end the run with, and a run that removes the package MUST leave no hold recorded for it.
  Why: a hold freezes an installed copy, so a hold and its package are one intent whenever the run is moving the package. apt records a hold for a package that is merely absent, which is the state `PKG-FR-APT-HELD-TARGET` calls bookkeeping and which blocks every later install of the name.
- **PKG-FR-APT-HELD-TARGET**: A package the target has and holds MUST NOT be proposed for install or upgrade, and MUST produce no package-level item. Its hold MUST still be an item. A hold naming a package the target does not have suppresses nothing: that package MUST be offered for install like any other.
  Why: apt records a hold for a package that is merely absent, so such a hold is bookkeeping rather than a state to protect. Reading it as one leaves the package missing on every future run.
...
- **PKG-FR-SNAP-HOLD**: A snap refresh hold MUST be an item of its own, both when it is added and when it is removed — EXCEPT where the snap is itself an item this run, an install or a change of revision or channel, in which case the two MUST be one question and the snap's decision MUST govern both (`PKG-FR-BLOCKS-REPLICATE`). A hold recorded for a snap the source no longer has MUST produce no item, and no command a sync issues may set a standing hold as a side effect.
...
- **PKG-FR-FLATPAK-FILTER**: A remote the source restricts with a filter MUST be replicated with that filter. The filter file MUST be copied byte-for-byte from the source to the same absolute path on the target and re-applied to the replicated remote. It is derived like a signing key and MUST NOT be a review item. No filter may be in force on a remote this run provisions while the approved applications from that remote install, whichever machine set it, and such a remote the source does not restrict MUST NOT stay restricted on the target. Both clauses reach only the remotes an approved application (or its dependency) needs: a remote no such application needs this run MUST keep whatever filter the target had, since `PKG-FR-FLATPAK-REMOTE-DERIVED` forbids syncing it at all. A filter that cannot be copied or re-applied MUST fail every approved application from that remote, naming the remote and the path.
  ...
  Why the ordering: a filter can be narrower than the set the source has installed, so one in force during the installs blocks the very replication it describes — the target's own as easily as the source's. Measured on Flatpak 1.14.6: installing a reference its remote's filter denies exits 1 with `Nothing matches <id> in remote <remote>` and lands nothing, while the same install of an allowed reference succeeds ([measurements](../adr/considerations/adr-020-flatpak-filter-and-trust-measurements.md)).
...
- **PKG-FR-FLATPAK-MASK**: Mask patterns MUST replicate per scope, added and removed alike, whether or not anything currently matches them, and MUST land after the applications. Editing or moving a pattern MUST be reported as found and MUST NOT be normalised. A mask MUST be an item of its own — EXCEPT where its pattern covers exactly one application that is itself an item this run in the same scope, in which case the two MUST be one question and the application's decision MUST govern both (`PKG-FR-BLOCKS-REPLICATE`). A mask riding an application's removal MUST still be applied.
  Why the count: a pattern covering two of a run's items would have to ride two questions whose answers can disagree, leaving no single decision to obey, and a pattern covering none is the ordinary case this article already replicates. Both keep an item of their own.
...
- **PKG-FR-COLLATERAL-MANUAL** can also ask about collateral the real command would never produce. Where the batch carries a hold the target kept for a package it no longer has, `Collateral.plan_time` simulates with `--allow-change-held-packages`, which `commands.install_args` withholds from the install itself; apt then reports what it would do to other held packages, which the real command refuses outright. Accepted: the question over-asks and never under-asks, which is the only way round that is safe.

### flatpak

- **PKG-FR-FLATPAK-FILTER**: `FlatpakSyncJob._clear_target_filters` takes every derived remote's filter off before the applications install and `_apply_remote_filters` puts the source's back afterwards, so a run whose re-apply fails leaves that remote unfiltered until the next run. Every approved application from it fails saying so, as the article requires, but until that next run the target offers more than either machine meant. Accepted: the alternative is installing behind a filter that may exclude the very applications being replicated.
- **PKG-FR-FLATPAK-REMOTE-TRUST** is met except where the source verifies a remote and holds no key material for it anywhere — neither a keyring of the remote's own nor a machine-level anchor `FlatpakSyncJob._stage_source_keys` can find. `_warn_about_trust` provisions the remote as the source has it and warns; installs from it will fail the signature check on the target as they already do on the source. Accepted, and stated by the article. One key source is outside what `_stage_source_keys` reads, so a remote holding its key that way is replicated as holding none: the ostree per-remote option `gpgkeypath`, which flatpak never writes and only a hand-edited repo config can set ([measurements](../adr/considerations/adr-020-flatpak-filter-and-trust-measurements.md)).

## Traceability

Every article above decomposes exactly one section of [Package sync — user requirements](package-sync-user-requirements.md). 130 articles: every one has a section, and every section that imposes obligations has articles. A new article needs a home here; a narrative section with no articles is either intentionally non-normative or a coverage gap.
