...
- **PKG-FR-SNAP-SIDELOAD**: A sideloaded snap MUST belong to `manual_snap_sync` alone. `snap_sync` MUST ignore it on both machines: never installed, never removed, never offered as an item, never the subject of a hold item, and never reported by that job. `manual_snap_sync` MUST offer the source's sideloads as findings resolvable by an install snippet, MUST identify one by the snap's name alone, and MUST NOT present one the target already has a snap of that name for, whatever revision or route put it there. The two jobs MUST decide what a sideload is by applying one shared rule, neither telling the other.
  Why `snap_sync` ignores them: no store can serve such a revision, so a snap that job cannot reinstall must not be one it offers to delete.
  Why the name alone: a sideload's revision changes on every reinstall from a newer file, so a revision inside the identity would orphan the user's snippet and their machine-specific mark and re-ask a question they had answered.
  Why any snap of that name counts as held: two machines' sideload revisions are independent install counters rather than two builds, and a store copy of the name is the same application by a route needing no snippet — replaying one over either would overwrite working software to chase a difference nothing here can read.
...
Every article above decomposes exactly one section of [Package sync — user requirements](package-sync-user-requirements.md). 137 articles: every one has a section, and every section that imposes obligations has articles. A new article needs a home here; a narrative section with no articles is either intentionally non-normative or a coverage gap.
