...
- **PKG-FR-HARMLESS-DEFAULT** is not met for an overwrite. `review._default_decision` starts a row at skip-once only for a removal or a repository conflict, so an `/etc/apt/apt.conf.d` file the target already holds is offered with *apply* preselected and a user who confirms without reading overwrites it. A snap whose revision or channel the run would change is preselected the same way; whether that counts as an overwrite is unruled.
- **PKG-FR-BATCHED** is not met for five kinds of item. `review._review_removal_group`, `_review_repo_conflict_group`, `_review_collateral_group` and `_review_unreproducible_group` each ask one item at a time, so a run offering four repository deletions asks four separate questions rather than one settled in a single pass. Each was ruled that way because the item's own content — a file body, two file bodies, an editor — has to be read immediately before the question. The article states no exception.
...
- **PKG-FR-COLLATERAL-MANUAL** is violated by a shipped bug. `collateral.Collateral.plan_time` builds `reviewed_names` from the candidate lists before any answer is given and `collateral.Collateral.classify` skips everything in it, so a package the user was offered for removal and answered *skip* loses its collateral protection: an approved install can then remove it with no question at all. The same method tests removals and downgrades only, so an approved change that would upgrade a manually-installed package raises nothing either.
- **PKG-FR-COLLATERAL-MARKED** is not implemented. `collateral.Collateral.protected` is the target's `apt-mark showmanual` set alone and consults no decision file, so a machine-specific package is never named as one.
- **PKG-FR-COLLATERAL-AUTO** is half implemented. Collateral touching only automatically-installed packages proceeds without asking, which is right, but `collateral.Collateral.classify` emits nothing for it and no other caller logs it, so the run keeps no record of a change nobody was asked about.
- **PKG-FR-REPO-DELETE** is not implemented. `diffing.diff_apt_sources` offers a target-only repository for deletion whatever still uses it; `messages.build_orphaned_packages_detail` discloses the machine-specific packages the deletion would strand instead of withholding the item.
- **PKG-FR-REPO-CONFLICT** asks more widely than required. `AptSyncJob._plan_repo_diffs` raises the conflict for every differing repository file feeding machine-specific software, without flatpak's gate on the file being one this run writes for an approved package (`flatpak_sync._capture_remote_conflicts`).
- **PKG-FR-APT-HOLD-VERSION** is not implemented. `commands.install_args` names the package alone and `packages.PackageConverger.hold` applies the hold afterwards, so a held package the target lacks is installed at whatever version the target offers and then frozen there. The requirement was ruled on after the code was written.
...
- **PKG-FR-FLATPAK-ORIGIN-DIFF** compares by name where no URL exists. `_same_vendor` falls back to comparing remote names when either machine's application names a remote it no longer configures. The fallback is deliberate and documented; the article forbids comparing names at all. Needs a ruling.
