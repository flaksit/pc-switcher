...
- **PKG-FR-FLATPAK-FILTER**: A remote the source restricts with a filter MUST be replicated with that filter. The filter file MUST be copied byte-for-byte from the source to the same absolute path on the target and re-applied to the replicated remote. It is derived like a signing key and MUST NOT be a review item. No filter may be in force on a remote this run provisions while the approved applications from that remote install, whichever machine set it, and a remote the source does not restrict MUST NOT stay restricted on the target. A filter that cannot be copied or re-applied MUST fail every approved application from that remote, naming the remote and the path.
  ...
  Why the ordering: a filter can be narrower than the set the source has installed, so one in force during the installs could block the very replication it describes — the target's own as easily as the source's.
