...
pc-switcher validates every enabled job before any of them changes anything — core behaviour, not specific to package sync. apt additionally refuses to start while the target's dpkg lock is held.
