...
Package sync is one job per package manager — apt, snap, flatpak — plus one job per kind of software no package manager can reproduce: hand-installed `.deb` packages, sideloaded snaps, flatpak applications no remote can supply, and unowned software under `/usr/local` and `/opt`. Each is enabled separately, reviewed separately, and can fail without stopping the others. Enabling one authorises pc-switcher to install and remove software on the target.
...
The system's own automatic apt updates are paused the same way, on both machines, whenever the run syncs apt — otherwise the machine could install packages underneath a sync that is deciding what it should have. Only updates actually running are touched: a machine whose automatic updates are already off is left as it is, and turned back on afterwards is only what was turned off. Stopping them carries no expiry of its own, so before stopping anything the run hands each machine an instruction to switch them back on a few hours later. That instruction belongs to the machine, not to pc-switcher, so a run that is killed or loses its connection still leaves updates that come back. A run finishing normally switches them back on itself and withdraws the instruction. If an earlier run died and left one outstanding, the next run pushes it past its own end rather than letting it fire mid-sync, and carries it out when it finishes.
...
A **dry run** plans and reviews exactly as a real run does — the questions are still asked — then changes nothing and records nothing. A **non-interactive run** — one with no terminal to answer at, such as from cron or a script — asks nothing and leaves skipped for this run every item that needed a decision, unless the command line answered that item in advance, and reports as skipped any job that held one.

Two options on the command line answer a review in advance, one per direction: one applies everything that would add software to the target, the other everything that would take software away. They answer as the source dictates, whatever the review screen's own starting answer would have been, and they reach every package job and nothing else. They answer only what the source's own state settles: a repository conflict over software the user marked as this machine's business, a file the target's own user wrote, an item that needs a snippet somebody has to write, and the Ubuntu Pro gate are all left alone, and a registry transfer that would lose an entry still ends the run. Nothing permanent is recorded and no snippet is authored — an unattended run may not decide that an item is specific to a machine. A job whose review they answered reports what it did; a job none of whose questions they answered still reports skipped. A job whose review held nothing to decide reports success instead: either it found nothing at all, or everything it found was reported rather than asked about, and no answer would have changed the outcome. Either way nothing is recorded and the snippet registry does not travel — it holds every snippet written on earlier runs, and sending it over the other machine's copy is a change nobody was there to approve.
...
Where both machines have the item and their copies differ, neither machine is the holder by right: either copy can be the one worth keeping, and the machines swap roles the moment the next sync is launched from the other end. So once the review is answered, every such item answered "always skip" is asked one further question, all of them together on one screen: whose own version is this — Atlas's, Vega's, or each machine's own? Naming one machine records the mark there; naming both records one on each, so the answer survives either machine losing its copy. Nothing travels for that item either way; what the answer decides is which machine's copy the decision is about, and therefore how long it lasts.
...
An application whose origin names no remote configured in its own scope is not this job's. It came from a local bundle or from a remote since deleted, so there is nowhere to fetch it from: it is never installed, never removed, and no remote is derived from its origin. Its own job offers it instead.
