...
**Created**: 2025-11-15
**Status**: Draft
**Input**: User description: "Write specs for features 1, 2 and 3 of the Feature breakdown.md: (1) Basic CLI & Infrastructure - Command parser, config system, SSH connection, logging, terminal UI skeleton, architecture for modular features; (2) Safety Infrastructure - Pre-sync validation framework, btrfs snapshot management, rollback capability; (3) Installation & Setup - Deploy to machines, dependency installation, btrfs subvolume structure setup"
...
4. **Given** snapshot creation fails on target (e.g., insufficient space), **When** the failure occurs, **Then** the snapshot module logs CRITICAL error with details, and the orchestrator aborts sync before any state changes occur
...
1. **Given** user configures `log_file_level: FULL` and `log_cli_level: INFO`, **When** sync runs and a module logs at DEBUG level, **Then** the message appears in the log file but not in terminal UI
...
- **FULL**: File-level operation details (e.g., "Copying /home/user/document.txt", "Created snapshot @home-presync-20251115-abc123")
- **DEBUG**: Verbose diagnostic information (e.g., command outputs, detailed timings, internal state transitions)
...
5. Checking that SSH connection to target was closed
...
1. **Given** sync operation is in progress with a module executing on target machine, **When** user presses Ctrl+C, **Then** the orchestrator catches SIGINT, logs "Sync interrupted by user" at WARNING level, calls the current module's cleanup() method, sends termination signal to target-side process via SSH, waits up to 5 seconds for graceful shutdown, then closes SSH connection and exits with code 130
...
3. **Given** user presses Ctrl+C multiple times rapidly, **When** the second SIGINT arrives before cleanup completes, **Then** orchestrator immediately force-terminates (kills SSH connection, exits with code 130) without waiting for graceful cleanup
...
- What happens when target has newer pc-switcher version than source?
  - Orchestrator detects version mismatch, logs WARNING "Target version 0.5.0 is newer than source 0.4.0, this is unusual", asks user to confirm proceeding or abort
...
- **FR-010** `[Reliability Without Compromise]`: System MUST create read-only btrfs snapshots of configured subvolumes on both source and target before any module executes (after version check)

- **FR-011** `[Reliability Without Compromise]`: System MUST create post-sync snapshots after all modules complete successfully

- **FR-012** `[Solid-State Stewardship]`: Snapshot naming MUST follow pattern `@<subvolume>-{presync|postsync}-<ISO8601-timestamp>-<session-id>` for clear identification and cleanup

- **FR-013** `[Reliability Without Compromise]`: Snapshot module MUST be marked as required and system MUST ignore attempts to disable it via configuration

- **FR-014** `[Frictionless Command UX]`: If pre-sync snapshot creation fails, system MUST log CRITICAL error and abort before any state modifications occur

- **FR-015** `[Reliability Without Compromise]`: System MUST provide rollback capability to restore from pre-sync snapshots (requires user confirmation)

- **FR-016** `[Solid-State Stewardship]`: System MUST provide snapshot cleanup command to delete old snapshots while retaining most recent N syncs

#### Logging System

- **FR-017** `[Documentation As Runtime Contract]`: System MUST implement six log levels: CRITICAL, ERROR, WARNING, INFO, FULL, DEBUG (in descending severity order)

- **FR-018** `[Reliability Without Compromise]`: Any log event at CRITICAL level MUST trigger immediate sync abort after calling cleanup() on current module

- **FR-019** `[Frictionless Command UX]`: System MUST support independent log level configuration for file output (`log_file_level`) and terminal display (`log_cli_level`)
...
- **FR-025** `[Reliability Without Compromise]`: On interrupt, system MUST send termination signal to any target-side processes via SSH and wait up to 5 seconds for graceful shutdown
