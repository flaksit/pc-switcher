...
class BtrfsSnapshotJob(Job):
    ...
    def __init__(self, phase: str = "presync"):
    ...
    async def validate(self, context: JobContext) -> List[str]:
        ...
        for subvol in subvolumes:
            # Check source
            res_src = await context.source.run_command(f"btrfs subvolume show /{subvol}")
            if not res_src.success:
                errors.append(f"Source subvolume /{subvol} not found or not btrfs")

            # Check target
            res_tgt = await context.target.run_command(f"btrfs subvolume show /{subvol}")
    ...
    async def execute(self, context: JobContext) -> None:
        ...
        context.logger.info(f"Creating {self.phase} snapshots for {subvolumes}")

        for subvol in subvolumes:
            snapshot_name = f"{subvol}-{self.phase}-{timestamp}-{session_id}"

            # Create on source
            context.logger.full(f"Creating source snapshot: {snapshot_name}")
            cmd_src = f"sudo btrfs subvolume snapshot -r /{subvol} /{snapshot_name}"
            res_src = await context.source.run_command(cmd_src)
            if not res_src.success:
                raise RuntimeError(f"Failed to create source snapshot {snapshot_name}: {res_src.stderr}")

            # Create on target
            context.logger.full(f"Creating target snapshot: {snapshot_name}")
            cmd_tgt = f"sudo btrfs subvolume snapshot -r /{subvol} /{snapshot_name}"
            res_tgt = await context.target.run_command(cmd_tgt)
            if not res_tgt.success:
                raise RuntimeError(f"Failed to create target snapshot {snapshot_name}: {res_tgt.stderr}")

        context.logger.info(f"Successfully created {self.phase} snapshots")
