...
class Orchestrator:
    ...
    def _check_package_jobs_precede_folder_sync(self) -> list[ConfigError]:
        ...
        `manual_deb_sync`, `manual_flatpak_sync` and `manual_installs_sync` are in the rule
        for the same reason as the three package managers: replaying an install snippet puts
        software on the target, and that software writes its own stock defaults on first
        appearance exactly as a package's postinst does.
