"""Unit tests for RemoteExecutor login_shell parameter and BashLoginRemoteExecutor subclass."""
...
class TestRunCommandLoginShell:
    ...
    async def test_login_shell_false_no_wrap(self) -> None:
        ...
        executor = RemoteExecutor(conn)
        await executor.run_command("echo hello", login_shell=False)
...
class TestBashLoginRemoteExecutor:
    """Test BashLoginRemoteExecutor subclass behavior."""

    def test_inherits_from_remote_executor(self) -> None:
        """BashLoginRemoteExecutor is a subclass of RemoteExecutor."""
        conn = MagicMock()
        executor = BashLoginRemoteExecutor(conn)
        assert isinstance(executor, RemoteExecutor)

    def test_default_login_shell_is_true(self) -> None:
        """BashLoginRemoteExecutor has login_shell=True by default."""
        conn = MagicMock()
        executor = BashLoginRemoteExecutor(conn)
        assert executor._default_login_shell is True

    def test_base_executor_default_is_false(self) -> None:
        """Base RemoteExecutor still defaults to False."""
        conn = MagicMock()
        executor = RemoteExecutor(conn)
        assert executor._default_login_shell is False

    @pytest.mark.asyncio
    async def test_run_command_uses_login_shell_by_default(self) -> None:
        """BashLoginRemoteExecutor wraps commands by default."""
        conn = MagicMock()
        conn.run = AsyncMock(return_value=MagicMock(exit_status=0, stdout="output", stderr=""))
    ...
    @pytest.mark.asyncio
    async def test_run_command_can_override_to_false(self) -> None:
        """Explicit login_shell=False overrides the default."""
        conn = MagicMock()
        conn.run = AsyncMock(return_value=MagicMock(exit_status=0, stdout="output", stderr=""))
    ...
    @pytest.mark.asyncio
    async def test_start_process_uses_login_shell_by_default(self) -> None:
        """BashLoginRemoteExecutor wraps start_process commands by default."""
        conn = MagicMock()
        mock_process = MagicMock()
        conn.create_process = AsyncMock(return_value=mock_process)
    ...
    @pytest.mark.asyncio
    async def test_start_process_can_override_to_false(self) -> None:
        """Explicit login_shell=False overrides the default for start_process."""
        conn = MagicMock()
        mock_process = MagicMock()
        conn.create_process = AsyncMock(return_value=mock_process)
