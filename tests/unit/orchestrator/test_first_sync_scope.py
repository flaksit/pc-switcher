...
class TestFirstSyncScopesPackageJobsOrdering:
    ...
    def test_manual_installs_sync_announces_snippet_replay_as_its_mechanism(self, mock_config: MagicMock) -> None:
        ...
        assert any("unowned installs under /usr/local and /opt" in item for item in scopes[0].scope_items)

    def test_manual_deb_sync_announces_the_packages_it_would_replay(self, mock_config: MagicMock) -> None:
        """The other unreproducible job announces its own scope, naming the hand-installed
        `.deb` packages rather than the paths `manual_installs_sync` speaks for."""
        mock_config.sync_jobs = {"manual_deb_sync": True}
        orchestrator, _ = _make_orchestrator(mock_config)
    ...
    def test_manual_snap_sync_announces_the_snaps_it_would_replay(self, mock_config: MagicMock) -> None:
        """Each unreproducible job announces its own scope, naming the sideloaded snaps
        rather than the packages, refs or paths the others speak for."""
    ...
    def test_manual_flatpak_sync_announces_the_refs_it_would_replay(self, mock_config: MagicMock) -> None:
        """G140 — the flatpak one names the refs no remote can supply rather than the
        packages, snaps or paths the others speak for."""
        mock_config.sync_jobs = {"manual_flatpak_sync": True}
        orchestrator, _ = _make_orchestrator(mock_config)

        scopes = orchestrator._first_sync_scopes()  # pyright: ignore[reportPrivateUsage]

        assert [scope.job_name for scope in scopes] == ["manual_flatpak_sync"]
        assert "snippet" in scopes[0].mechanism
        assert any("flatpak refs no remote can supply" in item for item in scopes[0].scope_items)
