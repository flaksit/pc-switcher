...
class TestDryRunContract:
    """The tool-wide `--dry-run` contract, on a real sync (ADR-014, `PKG-FR-KEY-COPY`)."""
    ...
    async def test_dry_run_previews_the_whole_pipeline_and_writes_nothing(
        ...
        """J53, J58, J59, C165, C168, C169, C2, A65 — ADR-014 / `PKG-FR-KEY-COPY`, one rehearsal, every
        ...
        try:
            ...
            assert source_after.sync_history == "__ABSENT__" and target_after.sync_history == "__ABSENT__", (
                "--dry-run recorded sync history (`PKG-FR-KEY-COPY`).\n"
