...
You just finished the planning phase for the feature. My quality assurance analyst reviewed your plan against the specs. You find their feedback a the file "spec-review-N*.md" in the feature folder. Make sure to read only the file with the highest number N.
First, critically read through their feedback. For each point in the feedback, ensure you understand the underlying issue. If you disagree with any point, discuss it with me. Come back to me and ask clarifying questions or ask for deciding if needed (e.g. between multiple possible options).
...
### Output
Write a report to a new file "spec-review-N-resolution.md", where N is the same number as of the review file you read. The goal is to have a trace of any decisions we make while resolving the feedback. The report should contain the following:
