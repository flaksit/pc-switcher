...
fix: "APPLIED in plan 01-18 (commit 5da5673, 'feat(01-18): add UI-routed log handler and TTY-aware setup_logging'). Replaced the `logging.StreamHandler(sys.stderr)` that wrote formatted log records straight to the terminal with a UILogHandler that routes records into TerminalUI's Recent Logs panel via `loop.call_soon_threadsafe`, so all log output now flows through the single rich.live.Live display instead of racing it. setup_logging() is TTY-aware, so the direct-stderr path is used only when no Live display owns the terminal."
verification: "CONFIRMED. Flooding/duplicate-frame behavior absent on a real interactive terminal during phase-01 human UAT (Test 7, 01-UAT.md). Diagnosis had been root-cause-only mode; the fix and its UAT verification were carried out in the normal GSD plan flow, not this debug session."
files_changed:
