...
Prompt wording constraints. Exactly two answers, labelled "I have attached the target — re-check and continue" and "Skip apt_sync this run (other jobs continue)". The body must name both files; state that the target reports no Ubuntu Pro attachment; give the two copy-paste commands to run **on the target** — `sudo pro attach <token from https://ubuntu.com/pro/dashboard>`, then `sudo pro enable esm-apps esm-infra` — and Ubuntu's own tutorial, https://documentation.ubuntu.com/pro/attach-tutorial/, which stays current if that procedure changes; and state that skipping leaves every other job running. The body must **not** account for the failure being prevented (public indexes, candidate selection, the eventual `401`): **ruled by the user**, replacing the earlier requirement to state it in one clause. It changes neither answer, so it is noise on the screen; §5.3.1 keeps it. No apology, no history, no reference to this document, nothing from the probe payload beyond the boolean. Untrusted content stays out of Rich markup (T-02-02) as everywhere else.
...
The VM test covers the skip arm only (`tests/integration/jobs/test_package_sync.py`, `TestTheESMAttachmentGateOnVMs`), and that is a statement about the fixtures rather than a gap: neither VM can be attached to Ubuntu Pro without putting the user's subscription token in CI, so the "attach now" arm has no VM to prove itself on. What the VM does prove, and no mocked-executor test can, is that the skip costs the whole job: a source-only `preferences.d` pin is in the always-sync bucket and must not reach the target either. That pin is the test's uuid-suffixed synthetic one rather than `ubuntu-pro-esm-apps` — `ubuntu-pro-client` ships `/etc/apt/preferences.d/ubuntu-pro-esm-apps` and `-esm-infra` on attached and unattached machines alike (`dpkg -L`, measured on both VMs), so the real ESM pins are identical on source and target and are never a pending write.
