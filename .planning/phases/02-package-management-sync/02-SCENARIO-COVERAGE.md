...
Requirement-derived scenario enumeration for the package jobs, mapped to pytest coverage. Branches come from ADR-020 (D-01…D-42), ADR-022, 02-208-HOLD-MASK-REPLICATION.md and `docs/planning/package-sync-conformance-criteria.md` — not from code paths, though code was read to decide what actually happens.
...
| F24 | Source remote carrying a `filtered` option | the filter file is copied byte-for-byte to the same absolute path on the target and re-applied there, after the approved applications land; the target's own filter comes off first; never a review item | U V | flat:`TestRemoteFilterReplicates` (14) — `test_the_filter_lands_after_the_app_it_could_exclude`, `test_the_filter_the_target_already_carries_comes_off_before_the_install`, `test_a_filter_the_source_no_longer_has_is_taken_off_the_target`, `test_the_filter_is_no_review_item`; INT:`test_a_source_filter_replicates_and_a_target_only_filter_comes_off` (two runs on the real Flathub: the file lands at the recorded path with the source's bytes, then a target-only filter comes off) |
