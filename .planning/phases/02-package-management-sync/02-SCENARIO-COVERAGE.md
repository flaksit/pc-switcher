...
| A7a | `Package:` stanza naming several packages, or the wildcard `*` | one fact per name; `*` yields none (it matches every package to apt and no name-keyed fact); one parser shared with the pin-file diff | U | apt:`TestPinStanzaParsing` (3) |
...
| F7 | Same-name, same-scope remote with a different URL | repointed as derived mechanism (`remote-modify --url`), silently, with no review line | U | flat:`test_a_differing_url_is_repointed_with_no_review_line`, `test_a_remote_the_target_already_matches_is_not_written_at_all` |
