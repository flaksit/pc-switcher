...
F1's run now proves remote trust travels rather than depending on a pre-seeded anchor (#215, rows F5a–F5d): a remote carries its GPG-verification setting and its own keyring, and the fixture trusts its repository only through the remote. The fixture's key is shared by both VMs and installed nowhere machine-wide, so the ref install after the sync fails unless pc-switcher imported the source's key.
