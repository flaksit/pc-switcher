...
- **D-03:** apt's manifest carries the **manually-installed set** (`apt-mark showmanual`, 153 packages on Atlas), not the full dpkg selection set (2306). apt resolves dependencies on the target. The set is curated by the machine-specific decision file (below).
...
- **D-23:** Snippets live in the **shared, synced config**, not the machine-local decision file: how to install something is knowledge about the package, not about the machine. Within its own single `execute()` run, `manual_installs_sync` both **pushes and applies** snippets: after its own review it writes any newly authored snippets to the source registry, sends `package-snippets.yaml` to the target with `send_file()`, and then **executes the snippet on the target for every detected item the user did not skip** — all in that same run. "Included" means *applied*, not merely transported: a snippet already in the source registry, and a snippet the user adds on the fly during the review (D-21), are BOTH replayed on the target this run. Whether an item is reproducible is judged by whether a snippet is available **from the source** (the machine being replicated), never by whether the target already holds it — the `send_file()` push happens before the apply step, so the target always has the snippet by the time it runs. The full sequence is: (1) detect manual installs on the source; (2) check which already have a snippet; (3) review the rest, offering skip-once / skip-always / add-snippet; (4) record skip-always to the machine-local decision file; (5) write and `send_file()` the snippet registry to the target; (6) execute the snippet on the target for every item not skipped. It must NOT travel via `config_sync`: that runs at SyncStep 9, before any review, so it cannot carry a snippet the user has not authored yet. It must also NOT rely on `folder_sync`: user sync jobs are in the user's hands and can be disabled or filtered, so no job's correctness may depend on another one running.
...
- **Normalizing the flatpak system/user scope split** — several runtimes are installed in both scopes on Atlas. Cleaning that up is a change to the machines, not a sync feature.
