...
expected: |
  ...
  - The folder mirror runs last, over a `/home` holding both machines' package state, and undoes none of the above: each machine keeps its own decision file and its own snippet registry, and the target is given user data only for the snap revision its own snapd holds (runbook §6, §7).
