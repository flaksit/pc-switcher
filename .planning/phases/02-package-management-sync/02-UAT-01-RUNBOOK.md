...
`manual_deb_sync`, `manual_snap_sync` and `manual_flatpak_sync` are deliberately absent from every config this runbook writes, which leaves them off — `sync_jobs` is read as written, with no merge against the shipped defaults. This runbook is the `manual_installs_sync` scenario; the other three, and everything else that came after them, are `02-UAT-02-RUNBOOK.md`'s.
...
- The collateral question is titled `Removing fortunes-min on pc2 would remove fortunes`, and the line under that title — above the keys, not under the row — says it is protected on both grounds, naming `pc2` as the machine the mark is on. It names `fortunes` and offers three answers on one row: the act `<y>`, hinted `remove fortunes-min from pc2, so fortunes is removed as well`; `skip now` `<s>`; and `stop the sync` `<q>`, hinted `nothing more is changed on pc2; what earlier jobs already did stays done`. There is no `<x>`.
- Only after those screens are confirmed does `Delete apt repositories pc1 no longer has` appear — a second round, asked because of the answers you just gave. It comes one file at a time and offers `99-pcsw-uat.sources` only, its detail reading `pc2 would stop getting software from https://***@packages.example.com/apt` — the password appears nowhere. `99-pcsw-inuse.sources` is not asked about at all.
...
- `Align snap package versions` follows, with two answers and no `<x>`, its act reading `align`: `hello`'s line reads `overwrites revision <pc2's> on pc2 with revision <pc1's> from pc1`, and the row starts applied rather than skipped.
...
- Its review then offers `/opt/pcsw-uat-app` and `/opt/pcsw-uat-vendor`, in that order, and nothing else — `/opt/pcsw-uat-empty`, `/usr/local/share/pcsw-uat-note` and `/usr/local/bin/pcsw-uat` are named nowhere, the last of them because pc2 already has it. Each item is asked on its own with three answers, titled `Install /opt/pcsw-uat-app on pc2?` and `Install /opt/pcsw-uat-vendor on pc2?`, and answering `<y>` opens **two** editors in sequence, each saying `(Ctrl-D to finish)`: `Install-or-update snippet for <item>:` first, then `Installed-version snippet for <item>:`. Submit an empty body, then one of only spaces, on each of the two: every one must be refused with `Neither snippet can be empty — enter both, or choose a skip.` and the three answers offered again.
...
The second must exit 1 and end with the `Job outcomes:` block — one line per job in execution order, each with a mark (`✔`, `⏭`, `✖`), the job name and its status. `manual_installs_sync` is `failed`, and the reason beside it is the command that failed on the scan of pc1's own `/opt`, quoted whole and ending `find: '/opt': Permission denied`. The other four jobs each reach their own outcome on their own line. That block is the only place the failure is printed: a second rendering of it anywhere is a finding.
...
Everything that arrived after this runbook was written, which `02-UAT-02-RUNBOOK.md` covers: the three snippet jobs beside `manual_installs_sync` and the exclusions they are the other half of; version convergence for an unreproducible item and the retry loop behind it; removals of unreproducible items; `--apply-package-installs` and `--apply-package-removals`; the machine-specific follow-up that asks whose copy a conflicting item is; and the apt update-timer pause. The two editors and the `Job outcomes:` block are the only ones of those this runbook meets, because they sit on paths it already walks.
