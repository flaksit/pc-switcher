...
- **D-03:** apt's manifest carries the manually-installed set (`apt-mark showmanual`, 153 packages on Atlas), not the full dpkg selection set (2306). apt resolves dependencies on the target. The set is curated by the machine-specific decision file.
...
- **D-18:** Items no package manager can reproduce are detected: apt packages with no repo candidate (4 on Atlas: `brscan3`, `brother-udev-rule-type1`, `cnpg`, `falco-app`) plus unowned installs under `/usr/local` and `/opt`.
