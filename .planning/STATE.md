...
- [Phase ?]: [Phase 2]: jobs/packages/review.py's interactive 'promote skip to permanent' prompt is implemented as a SECOND checkbox per actionable group over the entries the apply list left unticked (all-ticked group: no prompt), so D-07's third outcome reaches jobs/packages/sync_core.py's _record_permanent_skips from the UI. REPORT_ONLY groups are excluded by design: no holder machine, and a permanent mark on a version mismatch would stop the package syncing rather than stop reporting it.
...
- [Phase ?]: Plan 02-11: PACKAGE_REVIEW_AUTOMATION_ENV accepts SKIP_ALWAYS on a regular (non-unreproducible) item, matching the interactive never-offer-again checkbox in jobs/packages/review.py -- used to prove PackageSyncJob._record_permanent_skips/filter_inert's D-08 mechanism independent of the UI
